<?php 
    Class Tablexx2xxdibtestCompanyConsultantGrid { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "test_company_id"=>PDO::PARAM_INT, "consultant_id"=>PDO::PARAM_INT, "date_started"=>PDO::PARAM_STR, "position"=>PDO::PARAM_STR, "scope"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR);
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'dibexSubContainers_subCompanyConsultGrid') === 0) {
            ...
            if (array_key_exists('submitItemAlias_parent_id',$filterParams)) {
                ...
                if(is_array($value) || $value === ':submitItemAlias_parent_id')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_id' for filter 'dibexComponents_companyId' on dibtestCompanyConsultantGrid is missing from submitted values.");
        ...
        if(strpos($activeFilter, 'dibtestCompanyForm_dibtestCompanyConsultantGrid') === 0) {
            $crit = "`test_company_consultant`.`test_company_id` = :submitItemAlias_parent_id"; 
            if (array_key_exists('submitItemAlias_parent_id',$filterParams)) {
                $params[':submitItemAlias_parent_id'] = $filterParams['submitItemAlias_parent_id'];
                // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
        ...
        if(strpos($activeFilter, 'dibexEvents_CompanyConsultantGrid') === 0) {
            $crit = "`position` = 'team member'"; 
            $criteria .= " AND ($crit) ";
        }
        if(strpos($activeFilter, 'dibexEvents_Company1') === 0) {
            ...
            if (array_key_exists('submitItemAlias_parent_company1Id',$filterParams)) {
                ...
                if(is_array($value) || $value === ':submitItemAlias_parent_company1Id')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_company1Id' for filter 'dibexComponents_companyId' on dibtestCompanyConsultantGrid is missing from submitted values.");
                    $crit = '1 = 2'; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                else 
        ...
        if(strpos($activeFilter, 'dibexEvents_Supervisors') === 0) {
...
$sql .= $criteria . $orderStr . $limit;               
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(isset($attributes['notes'])) {            	
                ...
                return array ('error',"The 'Notes (notes)' field is required. Please provide a value.");
	        // position (plain text)
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for dibtestCompanyConsultantGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibtestCompanyConsultantGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if($result === FALSE || Database::count() === 0) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestCompanyConsultantGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestCompanyConsultantGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 7156, 'test_company_consultant', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx2xxdibtestCompanyConsultantGrid",
