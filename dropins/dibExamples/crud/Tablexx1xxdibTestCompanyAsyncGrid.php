<?php 
    Class Tablexx1xxdibTestCompanyAsyncGrid { 
    public $count = 0;
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "name"=>null, "chinese_name"=>null, "parent_company_id"=>null, "notes"=>null, "website"=>null, "parent_company_contact_id"=>null, "icon"=>null);
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id  are submitted to the server (ie they exist as (hidden) fields in container id 8063), or have defaults in pef_field.");
            ...
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 23312 are submitted to the server (ie they exist as fields in container id 8063)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
                if($makeUniqueValues)
            ...
            session_start();				
			$sess = session_id();
			if(isset($_SESSION['FileUploads']['dibTestCompanyAsyncGrid']['dib___NEWRECORD']['icon'])) {
				$newName = $_SESSION['FileUploads']['dibTestCompanyAsyncGrid']['dib___NEWRECORD']['icon'];
			    $file_name = $sess.'_8063_16590.tmp';
			    $file_path_expr = DIB::$USERSPATH.DIRECTORY_SEPARATOR.'tmp'.DIRECTORY_SEPARATOR.$file_name;
			    $newPath = FileUpload::getFileLocation(177114, $pkValues, $newName, 'dibTestCompanyAsyncGrid');
			    unset($_SESSION['FileUploads']['dibTestCompanyAsyncGrid']['dib___NEWRECORD']['icon']);
    ...
    public function delete($pkValues) {
        try {      
            ...
            $result = FileUpload::deleteFile('dibTestCompanyAsyncGrid',177114, $pkValues);
            if ($result === FALSE) 
            	Log::err("Item 177114 for container 'dibTestCompanyAsyncGrid' is configured in the Crud class to store files." .
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for dibTestCompanyAsyncGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibTestCompanyAsyncGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibTestCompanyAsyncGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibTestCompanyAsyncGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 8063, 'test_company', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibTestCompanyAsyncGrid",
