<?php 
    Class Tablexx1xxdibexComponents { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "unique_fld"=>PDO::PARAM_STR, "varchar10_required"=>PDO::PARAM_STR, "datetime_fld"=>PDO::PARAM_STR, "file_fld"=>PDO::PARAM_STR, "test_company_id"=>PDO::PARAM_INT, "test_company2_id"=>PDO::PARAM_INT, "image_fld"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "unique_fld"=>'none', "varchar10_required"=>'none', "datetime_fld"=>'none', "file_fld"=>'none', "test_company_id"=>'dropdown', "test_company2_id"=>'dropdown', "image_fld"=>'none');
    public $sqlFields = array();
    public $fkeyDisplay = array(
            'test_company_id'=>"^^CONCAT(`test_company1001`.`name`, ' (' , CAST(`test_company1001`.`id` AS CHAR), ')')^^", 
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test`.`id`,`test`.`varchar10_required`,`test`.`unique_fld`,`test`.`datetime_fld`,`test`.`file_fld`,`test`.`image_fld`,`test`.`test_company_id`,`test`.`test_company2_id` 
                , ^^CONCAT(company.name, ' (' , parentCompany.name , ')')^^ AS `test_company2_id_display_value`
                , ^^CONCAT(`test_company1001`.`name`, ' (' , CAST(`test_company1001`.`id` AS CHAR), ')')^^ AS `test_company_id_display_value`
                         FROM `test`                  
                LEFT JOIN test_company company  ON `test`.test_company2_id = company.id LEFT JOIN test_company parentCompany ON company.parent_company_id = parentCompany.id
                LEFT JOIN `test_company` `test_company1001` ON `test`.`test_company_id` = `test_company1001`.`id` 
                         WHERE $criteria";
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test`.`id` AS `Id`, `test`.`varchar10_required` AS `Varchar10 Required`, `test`.`unique_fld` AS `Unique Fld`, `test`.`datetime_fld` AS `Datetime Fld`, `test`.`file_fld` AS `File Uploader`, `test`.`image_fld` AS `Image File Uploader` 
                , ^^CONCAT(company.name, ' (' , parentCompany.name , ')')^^ AS `Company`
                , ^^CONCAT(`test_company1001`.`name`, ' (' , CAST(`test_company1001`.`id` AS CHAR), ')')^^ AS `Company Head-office`
            FROM `test` 
                LEFT JOIN test_company company  ON `test`.test_company2_id = company.id LEFT JOIN test_company parentCompany ON company.parent_company_id = parentCompany.id
                LEFT JOIN `test_company` `test_company1001` ON `test`.`test_company_id` = `test_company1001`.`id` 
                 ";
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "varchar10_required"=>null, "unique_fld"=>null, "datetime_fld"=>null, "file_fld"=>null, "image_fld"=>null, "test_company_id"=>null, "test_company2_id"=>null);
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(isset($attributes['test_company_id'])) {            	
	            if(trim((string)($attributes['test_company_id'])) !== '') {
		            if(!is_int((int)$attributes["test_company_id"]) || !ctype_digit((string)abs($attributes["test_company_id"])))
		                return array ('error',"The 'Company Head-office (test_company_id)' field must be an integer value.");
	            	if($attributes["test_company_id"] < 0.000000)
		                return array ('error',"The 'Company Head-office (test_company_id)' field value must be greater than or equal to 0.000000");
	            } 
	        }             
            ...
            if(isset($attributes['test_company2_id'])) {            	
	            if(trim((string)($attributes['test_company2_id'])) !== '') {
		            if(!is_int((int)$attributes["test_company2_id"]) || !ctype_digit((string)abs($attributes["test_company2_id"])))
		                return array ('error',"The 'Company (test_company2_id)' field must be an integer value.");
	            	if($attributes["test_company2_id"] < 0.000000)
		                return array ('error',"The 'Company (test_company2_id)' field value must be greater than or equal to 0.000000");
	            } 
	        }             
            //Check Unique Values for table option 23300
            $criteria = '1=1';
            if(!array_key_exists('unique_fld', $attributes)) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id  are submitted to the server (ie they exist as (hidden) fields in container id 7142), or have defaults in pef_field.");
            ...
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 23300 are submitted to the server (ie they exist as fields in container id 7142)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
                if($makeUniqueValues)
            ...
            // If a file was uploaded move the file to its folder
            session_start();				
			$sess = session_id();
			if(isset($_SESSION['FileUploads']['dibexComponents']['dib___NEWRECORD']['image_fld'])) {
				$newName = $_SESSION['FileUploads']['dibexComponents']['dib___NEWRECORD']['image_fld'];
			    $file_name = $sess.'_7142_14178.tmp';
			    $file_path_expr = DIB::$USERSPATH.DIRECTORY_SEPARATOR.'tmp'.DIRECTORY_SEPARATOR.$file_name;
			    $newPath = FileUpload::getFileLocation(154820, $pkValues, $newName, 'dibexComponents');
			    unset($_SESSION['FileUploads']['dibexComponents']['dib___NEWRECORD']['image_fld']);
			    if(is_file($file_path_expr)) rename($file_path_expr, $newPath[0]);
			}
			if(isset($_SESSION['FileUploads']['dibexComponents']['dib___NEWRECORD']['file_fld'])) {
				$newName = $_SESSION['FileUploads']['dibexComponents']['dib___NEWRECORD']['file_fld'];
			    $file_name = $sess.'_7142_14177.tmp';
			    $file_path_expr = DIB::$USERSPATH.DIRECTORY_SEPARATOR.'tmp'.DIRECTORY_SEPARATOR.$file_name;
			    $newPath = FileUpload::getFileLocation(154822, $pkValues, $newName, 'dibexComponents');
			    unset($_SESSION['FileUploads']['dibexComponents']['dib___NEWRECORD']['file_fld']);
			    if(is_file($file_path_expr)) rename($file_path_expr, $newPath[0]);
			}
            session_write_close();			
            $crit = TRUE;
    ...
    public function update($pkValues, $attributes) {
        ...
        try {    
            ...
            $fieldType[":pk1"] = PDO::PARAM_INT;            
            $pkCrit = "`test`.`id` = :pk1";                   
            // Check Unique Values for table option unique_fld(23300)
            $criteria = '1=1 ';           
            if(!array_key_exists('unique_fld', $attributes)) {
                $uRst = Database::fetch("SELECT `unique_fld` FROM `test` WHERE $pkCrit", $params);
                $attributes['unique_fld'] = $uRst['unique_fld'];
            }
            $criteria .= " AND `unique_fld` = :fk1 ";
            $criteria .= "AND `id` <> :pk1";
            $sql = "SELECT `test`.`id` AS pkv FROM `test` WHERE $criteria";
            ...
            if(!empty($rst)) {
                if(count($paramsU) > 1)
                    ...
                    return array('error',"Update record cancelled. The value in 'unique_fld' needs to be unique. Another record already contains the same value.");
            }            
            $crit = $pkCrit;
    ...
    public function delete($pkValues) {
        try {      
            ...
            $result = FileUpload::deleteFile('dibexComponents',154820, $pkValues);
            if ($result === FALSE) 
            	Log::err("Item 154820 for container 'dibexComponents' is configured in the Crud class to store files." .
            ...
            $result = FileUpload::deleteFile('dibexComponents',154822, $pkValues);
            if ($result === FALSE) 
            	Log::err("Item 154822 for container 'dibexComponents' is configured in the Crud class to store files." .
            			 "\r\nAttempt to resolve configuration and delete physical files to this item for primary key values '" . json_encode($pkValues) . "' failed.\r\n" .
            			 "Establish if the Crud file needs to be regenerated (perhaps the item no longer stores files, or if the webserver user has rights to delete these files.");
            $result = FileUpload::deleteFile('dibexComponents',223346, $pkValues);
            if ($result === FALSE) 
            	Log::err("Item 223346 for container 'dibexComponents' is configured in the Crud class to store files." .
            			 "\r\nAttempt to resolve configuration and delete physical files to this item for primary key values '" . json_encode($pkValues) . "' failed.\r\n" .
            			 "Establish if the Crud file needs to be regenerated (perhaps the item no longer stores files, or if the webserver user has rights to delete these files.");
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `varchar10_required`, `unique_fld`, `datetime_fld`, `file_fld`, `image_fld`, `test_company_id`, `test_company2_id`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for dibexComponents.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexComponents records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexComponents' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexComponents' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 7142, 'test', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getCaptions() {
    	return array('Id', 'Unique Fld', 'Varchar10 Required', 'Datetime Fld', 'File Uploader', 'Company Head-office', 'Company', 'Image File Uploader' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexComponents",
					 'selectFields' => "`test`.`id`,`test`.`varchar10_required`,`test`.`unique_fld`,`test`.`datetime_fld`,`test`.`file_fld`,`test`.`image_fld`,`test`.`test_company_id`,`test`.`test_company2_id`",
