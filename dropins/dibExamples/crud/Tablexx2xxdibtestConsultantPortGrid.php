<?php 
    Class Tablexx2xxdibtestConsultantPortGrid { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "name"=>PDO::PARAM_STR, "employed_at_id"=>PDO::PARAM_INT, "email"=>PDO::PARAM_STR, "address1"=>PDO::PARAM_STR, "address2"=>PDO::PARAM_STR, "city_town"=>PDO::PARAM_STR, "country"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "name"=>'none', "employed_at_id"=>'dropdown', "email"=>'none', "address1"=>'none', "address2"=>'none', "city_town"=>'none', "country"=>'none', "notes"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
        ...
        if(strpos($activeFilter, 'dibtestCompanyForm_dibtestConsultantGrid') === 0) {
            ...
            if (array_key_exists('submitItemAlias_parent_id',$filterParams)) {
                ...
                if(is_array($value) || $value === ':submitItemAlias_parent_id')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_id' for filter 'dibexComponents_companyId' on dibtestConsultantPortGrid is missing from submitted values.");
        ...
        if(strpos($activeFilter, 'dibexSubContainers_divPortItem') === 0) {
            $crit = "`country` = 'USA'"; 
            $criteria .= " AND ($crit) ";
        }
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test_consultant`.`id`,`test_consultant`.`name`,`test_consultant`.`notes`,`test_consultant`.`email`,`test_consultant`.`address1`,`test_consultant`.`address2`,`test_consultant`.`city_town`,`test_consultant`.`country`,`test_consultant`.`employed_at_id` 
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_consultant`.`id` AS `Id`, `test_consultant`.`name` AS `Name`, `test_consultant`.`notes` AS `Notes`, `test_consultant`.`email` AS `Email`, `test_consultant`.`address1` AS `Address1`, `test_consultant`.`address2` AS `Address2`, `test_consultant`.`city_town` AS `City Town`, `test_consultant`.`country` AS `Country` 
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "name"=>null, "notes"=>null, "email"=>null, "address1"=>null, "address2"=>null, "city_town"=>null, "country"=>null, "employed_at_id"=>null);
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id  are submitted to the server (ie they exist as (hidden) fields in container id 7166), or have defaults in pef_field.");
            ...
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 23321 are submitted to the server (ie they exist as fields in container id 7166)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
                if($makeUniqueValues)
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `name`, `notes`, `email`, `address1`, `address2`, `city_town`, `country`, `employed_at_id`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for dibtestConsultantPortGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibtestConsultantPortGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestConsultantPortGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestConsultantPortGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 7166, 'test_consultant', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getCaptions() {
    	return array('Id', 'Name', 'Employed At', 'Email', 'Address1', 'Address2', 'City Town', 'Country', 'Notes' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx2xxdibtestConsultantPortGrid",
					 'selectFields' => "`test_consultant`.`id`,`test_consultant`.`name`,`test_consultant`.`notes`,`test_consultant`.`email`,`test_consultant`.`address1`,`test_consultant`.`address2`,`test_consultant`.`city_town`,`test_consultant`.`country`,`test_consultant`.`employed_at_id`",
