<?php 
    Class Tablexx2xxdibtestConsultantGrid { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "name"=>PDO::PARAM_STR, "employed_at_id"=>PDO::PARAM_INT, "dibsqli_insertedSql"=>PDO::PARAM_STR, "dibsqli_moreInsertedSql"=>PDO::PARAM_STR, "mobile"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR, "address1"=>PDO::PARAM_STR, "address2"=>PDO::PARAM_STR, "city_town"=>PDO::PARAM_STR, "country"=>PDO::PARAM_STR, "longitude"=>PDO::PARAM_STR, "latitude"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "name"=>'none', "employed_at_id"=>'dropdown', "dibsqli_insertedSql"=>'dibsqli', "dibsqli_moreInsertedSql"=>'dibsqli', "mobile"=>'none', "email"=>'none', "address1"=>'none', "address2"=>'none', "city_town"=>'none', "country"=>'none', "longitude"=>'none', "latitude"=>'none', "notes"=>'none');
    public $sqlFields = array("dibsqli_insertedSql"=>"(SELECT count(*) FROM test_company_consultant WHERE consultant_id = test_consultant.id)", "dibsqli_moreInsertedSql"=>"^^CONCAT(test_consultant.name, ' - ', test_consultant.id)^^");
    ...
    private function parseFilterArray($filterParams=array(), &$params=array(), &$fieldType=array()) {
        ...
        foreach($filterParams as $record) {
            ...
            if(array_key_exists($field, $this->sqlFields))
            	...
            	$str .= '`test_consultant`.`' . $field . '` = :pk' . $i . ' AND ';
            $params[':pk' . $i] = $value;
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'dibexEvents_dibtestConsultantGrid') === 0) {
            $crit = "employed_at_id = :submitItemAlias_parent_companyId"; 
            if (array_key_exists('submitItemAlias_parent_companyId',$filterParams)) {
                $params[':submitItemAlias_parent_companyId'] = $filterParams['submitItemAlias_parent_companyId'];
                ...
                // unset($filterParams["submitItemAlias_parent_companyId"]);
            } else {
                $value = EvalCriteria::evalParam(':submitItemAlias_parent_companyId', $filterParams);
                if(is_array($value) || $value === ':submitItemAlias_parent_companyId')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_companyId' for filter 'dibexComponents_companyId' on dibtestConsultantGrid is missing from submitted values.");
    ...
    public function getToolbarInfo($pkValues, $activeFilter, $filterParams, $getFirstOnly=FALSE) {        
        ...
        $sql = "SELECT count(*) as dib__total, min(`test_consultant`.`id`) as dib__minId, max(`test_consultant`.`id`) as dib__maxId FROM $fromClause $criteria";
        $rst = dibMySqlPdo::execute($sql, DIB::$CONTAINERDATA[2], $params, true);
        if(empty($rst))
        ...
        if($totalCount>0) {
            ...
            $tempCrit = ($criteria === '') ? "WHERE `test_consultant`.`id` < :pk1" : "$criteria AND `test_consultant`.`id` < :pk1";
            $sql = "SELECT max(`test_consultant`.`id`) as dib__Id, count(`test_consultant`.`id`) as dib__counter FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, DIB::$CONTAINERDATA[2], $params, true);
            if(empty($rst))
            ...
            // Get next
            $tempCrit = ($criteria === '') ? "WHERE `test_consultant`.`id` > :pk1" : "$criteria AND `test_consultant`.`id` > :pk1";
            $sql = "SELECT min(`test_consultant`.`id`) as dib__Id FROM $fromClause $tempCrit";
            $rst = dibMySqlPdo::execute($sql, DIB::$CONTAINERDATA[2], $params, true);
            if(empty($rst))
...
$sql = "SELECT `test_consultant`.`id` 
        FROM $fromClause
        $criteria
        ORDER BY `test_consultant`.`id` 
        LIMIT " . ($position - 1) . ', 1'; 
        $rst = dibMySqlPdo::execute($sql, DIB::$CONTAINERDATA[2], $params, true);
        if(empty($rst))
            return null;
    ...
    public function parseGridFilter($gridFilter, &$params=array(), &$fieldType=array(), &$usedSqlField=FALSE) {
        ...
        foreach($gridFilter as $row) {
            ...
            if(array_key_exists($field, $this->sqlFields)) {
            	...
            	$fieldExpr = "`test_consultant`.`$field`";
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`test_consultant`.`id` = :pk1 ";
                $sql = "SELECT `test_consultant`.`id`,`test_consultant`.`name`,`test_consultant`.`notes`,`test_consultant`.`mobile`,`test_consultant`.`email`,`test_consultant`.`address1`,`test_consultant`.`address2`,`test_consultant`.`city_town`,`test_consultant`.`country`,`test_consultant`.`longitude`,`test_consultant`.`latitude`,`test_consultant`.`employed_at_id` 
            , (SELECT count(*) FROM test_company_consultant WHERE consultant_id = test_consultant.id) AS `dibsqli_insertedSql`, ^^CONCAT(test_consultant.name, ' - ', test_consultant.id)^^ AS `dibsqli_moreInsertedSql`
                , ^^CONCAT(`test_company1001`.`name`, ' (' , CAST(`test_company1001`.`id` AS CHAR), ')')^^ AS `employed_at_id_display_value`
                ...
                if($page === 1 || $countMode==='all'){
                	$sql = "SELECT Count(*) AS totalcount FROM `test_consultant`  ";
            ...
            ";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
	                    $sql = "SELECT Count(*) AS filteredCount FROM `test_consultant` $join  $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);                          
...
if($readType === 'exportlist')
    $sql = "SELECT 
                ...
                , ^^CONCAT(`test_company1001`.`name`, ' (' , CAST(`test_company1001`.`id` AS CHAR), ')')^^ AS `Employed At`
            , (SELECT count(*) FROM test_company_consultant WHERE consultant_id = test_consultant.id) AS `Inserted SQL`, ^^CONCAT(test_consultant.name, ' - ', test_consultant.id)^^ AS `More Inserted SQL`
            FROM `test_consultant` 
                LEFT JOIN `test_company` `test_company1001` ON `test_consultant`.`employed_at_id` = `test_company1001`.`id` 
                 ";
else
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "name"=>null, "notes"=>null, "mobile"=>null, "email"=>null, "address1"=>null, "address2"=>null, "city_town"=>null, "country"=>null, "longitude"=>null, "latitude"=>null, "employed_at_id"=>null);
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
	        ...
	        // address1 (plain text)
            if(isset($attributes['address1'])) {            	
	            if(trim((string)($attributes['address1'])) !== '') {
	            } 
	        }             
	        // address2 (plain text)
            if(isset($attributes['address2'])) {            	
	            if(trim((string)($attributes['address2'])) !== '') {
	            } 
	        }             
	        // city_town (plain text)
            if(isset($attributes['city_town'])) {            	
	            if(trim((string)($attributes['city_town'])) !== '') {
	            } 
	        }             
	        // country (plain text)
            if(isset($attributes['country'])) {            	
	            if(trim((string)($attributes['country'])) !== '') {
	            } 
	        }             
	        // longitude (decimal)
            if(isset($attributes['longitude'])) {            	
	            if(trim((string)($attributes['longitude'])) !== '') {
					if(!is_numeric($attributes["longitude"]))
						return array ('error',"The 'Longitude (longitude)' field must be a valid integer or decimal value.");
			    	$d = (string)(double)$attributes["longitude"];
            ...
            if(isset($attributes['latitude'])) {            	
	            if(trim((string)($attributes['latitude'])) !== '') {
					if(!is_numeric($attributes["latitude"]))
						return array ('error',"The 'Latitude (latitude)' field must be a valid integer or decimal value.");
            ...
            if(isset($attributes['employed_at_id'])) {            	
	            if(trim((string)($attributes['employed_at_id'])) !== '') {
		            if(!is_int((int)$attributes["employed_at_id"]) || !ctype_digit((string)abs($attributes["employed_at_id"])))
		                return array ('error',"The 'Employed At (employed_at_id)' field must be an integer value.");
	            } 
	        }             
            //Check Unique Values for table option 23321
            $criteria = '1=1';
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id  are submitted to the server (ie they exist as (hidden) fields in container id 7147), or have defaults in pef_field.");
            ...
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 23321 are submitted to the server (ie they exist as fields in container id 7147)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
                if($makeUniqueValues)
                    // Force unique values - for combinations, only enforce on first 
                    $attributes['name'] = SyncFunctions::cleanName($attributes['name'],'test_consultant', '');
            ...
            $sql = "INSERT INTO `test_consultant` (";
            $fieldList = '';
            $valueList = '';
            $fieldType = array();
            $i=0;
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue)
                    $this->auditInsert("create", $fieldName, null, $newValue, 1442, $recordId);
    ...
    public function update($pkValues, $attributes) {
        ...
        try {    
            ...
            // Check Unique Values for table option name(23321)
            $criteria = '1=1 ';           
            if(!array_key_exists('name', $attributes)) {
                $uRst = Database::fetch("SELECT `name` FROM `test_consultant` WHERE $pkCrit", $params);
            ...
            $sql = 'UPDATE `test_consultant` SET ';
            $i=0;
            ...
            if ($crit===TRUE) {
                ...
                foreach ($validAttributes AS $fieldName => $newValue) {
                    if ($newValue !== $recordOld[$fieldName])
                        $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 1442, $recordId);
                }
            } elseif (is_array($crit)) return $crit;
        } catch (Exception $e) {            
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `name`, `notes`, `mobile`, `email`, `address1`, `address2`, `city_town`, `country`, `longitude`, `latitude`, `employed_at_id`
                    FROM `test_consultant` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for dibtestConsultantGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibtestConsultantGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestConsultantGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestConsultantGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = 'UPDATE test_consultant SET `' . $args[0] . "` = :value WHERE $pkCrit";					
					PeffApp::$array['DuplicateRecords']['test_consultant*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    public function getCaptions() {
    	return array('Id', 'Name', 'Employed At', 'Inserted SQL', 'More Inserted SQL', 'Mobile', 'Email', 'Address1', 'Address2', 'City Town', 'Country', 'Longitude', 'Latitude', 'Notes' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx2xxdibtestConsultantGrid",
					 'selectFields' => "`test_consultant`.`id`,`test_consultant`.`name`,`test_consultant`.`notes`,`test_consultant`.`mobile`,`test_consultant`.`email`,`test_consultant`.`address1`,`test_consultant`.`address2`,`test_consultant`.`city_town`,`test_consultant`.`country`,`test_consultant`.`longitude`,`test_consultant`.`latitude`,`test_consultant`.`employed_at_id`",
