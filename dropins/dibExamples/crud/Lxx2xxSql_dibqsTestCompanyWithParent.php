...
class Lxx2xxSql_dibqsTestCompanyWithParent {    
	...
	*/
    public function getList($containerItemId, $page=1, $page_size, $query=null, $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        ...
        $permCrit = '';
        // Set sql parts                  
        $totalSql = "SELECT Count(*) AS totalcount FROM test_company company LEFT JOIN test_company parentCompany ON company.parent_company_id = parentCompany.id ";             
        $sql = "    company.id AS `id` ,  ^^CONCAT(company.name, ' (' , parentCompany.name , ')')^^ AS id_display_value ";
        $from_clause = "test_company company LEFT JOIN test_company parentCompany ON company.parent_company_id = parentCompany.id";
        $display_field = "^^CONCAT(company.name, ' (' , parentCompany.name , ')')^^";
        $pkey = "company.id";        
        $order_by = ($phpFilter==='') ? " ORDER BY company.`name`" : ''; 
        $criteria = "parentCompany.id is NOT NULL";  
        // Handle filters
        if ($activeFilter){
            $userFilter = '';
            if($containerItemId === '154825' && $activeFilter === 'dibexComponents_companyId') {
                $userFilter = "company.parent_company_id = :submitItemAlias_self_parentCompanyId"; 
