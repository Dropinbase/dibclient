<?php 
    ...
    public function getToolbarInfo($pkValues, $option, $activeFilter, $filterParams) {
        ...
        $pkList = "`test_consultant`.`id`";
        $criteria = '';
        $order ='';
        if ($option === 'next')  { 
            $criteria = "WHERE `id` > :pk1";
            $order = "ORDER BY `test_consultant`.`id`";
        } elseif ($option === 'prev') {
            $order = "ORDER BY `id` DESC";
            $criteria = "WHERE `id` < :pk1";
        } elseif ($option === 'last') {
            $order = "ORDER BY `test_consultant`.`id` DESC";
            $params = array();
            $fieldType = array();
        } elseif ($option === 'total') {
            ...
            $order = "ORDER BY `test_consultant`.`id` ASC";
            $params = array();
            $fieldType = array();
        }
        // activeFilter                
        if(!empty($activeFilter)){  
            $crit = ''; 
            if($activeFilter === 'dibexSubContainers_subConsultForm') {  
                $crit = "test_consultant.id = :submitLastRecord_subCompanyConsultGrid_consultant_id"; 
                if (array_key_exists('submitLastRecord_subCompanyConsultGrid_consultant_id',$filterParams)) {
                    $params[':submitLastRecord_subCompanyConsultGrid_consultant_id'] = $filterParams['submitLastRecord_subCompanyConsultGrid_consultant_id'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitLastRecord_subCompanyConsultGrid_consultant_id', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitLastRecord_subCompanyConsultGrid_consultant_id' for filter 'subConsultForm' on dibtestConsultantForm is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitLastRecord_subCompanyConsultGrid_consultant_id'] = $value;
                }
...
$sql = "SELECT $pkList FROM `test_consultant` $criteria $order LIMIT 1";
