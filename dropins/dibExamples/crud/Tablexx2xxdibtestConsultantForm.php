<?php 
    Class Tablexx2xxdibtestConsultantForm { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "address1"=>PDO::PARAM_STR, "name"=>PDO::PARAM_STR, "address2"=>PDO::PARAM_STR, "longitude"=>PDO::PARAM_STR, "employed_at_id"=>PDO::PARAM_INT, "mobile"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR, "city_town"=>PDO::PARAM_STR, "latitude"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR, "country"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "address1"=>'none', "name"=>'none', "address2"=>'none', "longitude"=>'none', "employed_at_id"=>'dropdown', "mobile"=>'none', "email"=>'none', "city_town"=>'none', "latitude"=>'none', "notes"=>'none', "country"=>'none');
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'dibexSubContainers_subConsultForm') === 0) {
            $crit = "test_consultant.id = :submitLastRecord_subCompanyConsultGrid_consultant_id"; 
            if (array_key_exists('submitLastRecord_subCompanyConsultGrid_consultant_id',$filterParams)) {
                $params[':submitLastRecord_subCompanyConsultGrid_consultant_id'] = $filterParams['submitLastRecord_subCompanyConsultGrid_consultant_id'];
                ...
                // unset($filterParams["submitLastRecord_subCompanyConsultGrid_consultant_id"]);
            } else {
                ...
                if(is_array($value) || $value === ':submitLastRecord_subCompanyConsultGrid_consultant_id')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitLastRecord_subCompanyConsultGrid_consultant_id' for filter 'dibexComponents_companyId' on dibtestConsultantForm is missing from submitted values.");
...
$sql .= $criteria . $orderStr . $limit;               
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id  are submitted to the server (ie they exist as (hidden) fields in container id 7157), or have defaults in pef_field.");
            ...
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 23321 are submitted to the server (ie they exist as fields in container id 7157)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
                if($makeUniqueValues)
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for dibtestConsultantForm.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibtestConsultantForm records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestConsultantForm' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestConsultantForm' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 7157, 'test_consultant', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getCaptions() {
    	return array('Id', 'Address1', 'Name', 'Address2', 'Longitude', 'Employed At', 'Mobile', 'Email', 'City Town', 'Latitude', 'Notes', 'Country' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx2xxdibtestConsultantForm",
