<?php 
    Class Tablexx1xxdibtestGridTree { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "name"=>PDO::PARAM_STR, "chinese_name"=>PDO::PARAM_STR, "parent_company_id"=>PDO::PARAM_INT, "website"=>PDO::PARAM_STR, "parent_company_contact_id"=>PDO::PARAM_INT, "icon"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "name"=>'none', "chinese_name"=>'none', "parent_company_id"=>'dropdown', "website"=>'none', "parent_company_contact_id"=>'dropdown', "icon"=>'none', "notes"=>'none');
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                // Add Treedata criteria coming from $node being expanded
                $treeCrit = '';
                if (strcasecmp('root', $node) === 0) 
                    // Get root level nodes 
                    $treeCrit = "`test_company`.`parent_company_id` IS NULL";
                elseif (!empty($node)) {
                    // Get child nodes
                    if($node !=(string)(int)$node) return array(); // stop invalid values/hackers
                    $treeCrit = "`test_company`.`parent_company_id` = " . $node;
                }
                if($treeCrit !== '')
                    $criteria .= ' AND (' . $treeCrit . ')';  
                if ($criteria !== '') {
            ...
            ";
                ...
                $orderStr =  " ORDER BY `test_company`.`id`";
                if($order) {
                    $orderStr = " ORDER BY ";
                ...
                // Set SQL statement
                    // Template: MySql - SQL for paging purposes for trees - only the top level is paged
    if($node !== 'root')
        $limit = '';
    elseif($page === 1)
...
if($readType === 'exportlist')
    ...
    $sql = "SELECT `test_company`.`id`,`test_company`.`name`,`test_company`.`chinese_name`,`test_company`.`parent_company_id`,`test_company`.`notes`,`test_company`.`website`,`test_company`.`parent_company_contact_id`,`test_company`.`icon` 
            ...
            FROM `test_company` 
                ...
                LEFT JOIN `test_consultant` `test_consultant1002` ON `test_company`.`parent_company_contact_id` = `test_consultant1002`.`id` 
                 ";   
$sql .= $criteria . $orderStr  . ", `test_company`.`id` $limit" ;               
                dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);
                ...
                if($attributes) {
	                // Add the leaf attribute for treedata - get list of page's pkey values - then get associated leaf counts
	                $crit = '';
	                foreach ($attributes as $row)
	                	$crit .= $row['id'] . ', ';
	                $crit = rtrim($crit, ', ');
	                $isLeafSql = "SELECT `test_company`.`id` AS dib___Pk, Count(recursively_Joined_TableX.id) AS dib___Counter 
	                              FROM `test_company` LEFT JOIN `test_company` recursively_Joined_TableX 
	                                   ON `test_company`.`id` = recursively_Joined_TableX.`parent_company_id`
	                              WHERE `test_company`.`id` IN ($crit)
	                              GROUP BY `test_company`.`id` ";
	                $leafCounts = dibMySqlPdo::execute($isLeafSql, DIB::$CONTAINERDATA[2], $params, false);
	                $tempCounts = array();
	                foreach ($leafCounts as $row)
	                	$tempCounts[$row['dib___Pk']] = $row['dib___Counter'];
	                foreach($attributes as $key=>$row)
	                    $attributes[$key]['leaf'] = ($tempCounts[$row['id']] > 0) ? FALSE : TRUE;
                }
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(isset($attributes['chinese_name'])) {            	
                ...
                return array ('error',"The 'Chinese Name (chinese_name)' field is required. Please provide a value.");
            ...
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id  are submitted to the server (ie they exist as (hidden) fields in container id 9564), or have defaults in pef_field.");
            ...
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 23312 are submitted to the server (ie they exist as fields in container id 9564)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
                if($makeUniqueValues)
            ...
            session_start();				
			$sess = session_id();
			if(isset($_SESSION['FileUploads']['dibtestGridTree']['dib___NEWRECORD']['icon'])) {
				$newName = $_SESSION['FileUploads']['dibtestGridTree']['dib___NEWRECORD']['icon'];
			    $file_name = $sess.'_9564_16590.tmp';
			    $file_path_expr = DIB::$USERSPATH.DIRECTORY_SEPARATOR.'tmp'.DIRECTORY_SEPARATOR.$file_name;
			    $newPath = FileUpload::getFileLocation(242612, $pkValues, $newName, 'dibtestGridTree');
			    unset($_SESSION['FileUploads']['dibtestGridTree']['dib___NEWRECORD']['icon']);
    ...
    public function delete($pkValues) {
        try {      
            ...
            $sql = "SELECT * FROM `test_company` WHERE $pkCrit";
            ...
            $result = FileUpload::deleteFile('dibtestGridTree',242612, $pkValues);
            if ($result === FALSE) 
            	Log::err("Item 242612 for container 'dibtestGridTree' is configured in the Crud class to store files." .
            ...
            $sql = "DELETE FROM `test_company` WHERE $crit";
            dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);       
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) 
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 1440, $recordId);
                } elseif (is_array($crit)) return $crit;
                return true;
            }
        }  catch (Exception $e) {        
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for dibtestGridTree.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibtestGridTree records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestGridTree' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestGridTree' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
    ...
    function dropNode($dropPosition, $dropNodeId, $nodeId, $parentId) {
        $params = array();
        if ($dropPosition !== 'append' && (is_null($parentId) || $parentId === 'root')) {
            $parentCondition = "`parent_company_id` IS NULL";
            $params = array(':parentId' => NULL);
        } else {
            if($dropPosition === 'append') $parentId = $dropNodeId;
            $parentCondition = "`parent_company_id` = :parentId";
            $params = array(':parentId' => $parentId);
        }
        // Get new position
        if($dropPosition === 'append') {
            $sql = "SELECT MAX(`test_company`.`id`) AS order__Pos FROM `test_company`
                    WHERE " . $parentCondition;
            $dropPosition = 'after';
            $rst = dibMySqlPdo::execute($sql, DIB::$CONTAINERDATA[2], $params, true);
        } else {
            $sql = "SELECT `test_company`.`id` AS order__Pos FROM `test_company`
                    WHERE `test_company`.`id` = :dropNodeId";            
            $rst = dibMySqlPdo::execute($sql, DIB::$CONTAINERDATA[2], array(':dropNodeId' => $dropNodeId), true);
        }  
        if(empty($rst) || count($rst) !== 1)
            return array('error','Invalid request to server. Your last action may not be saved.'); 
        $orderNo = (is_null($rst['order__Pos'])) ? 10 : $rst['order__Pos'];
        if ($dropPosition === 'after')
            $orderOperator = '>';
        else
            $orderOperator = '>=';
         // ***TODO change to crud actions OR check permissions on orderNo field and record criteria and handle AuditTrail... drag-and-drop is actually bulk-update permissions ?
        $sql = "UPDATE `test_company` SET `test_company`.`id` = `test_company`.`id` + 10 
                WHERE $parentCondition AND `test_company`.`id` $orderOperator $orderNo";
        $rst = dibMySqlPdo::execute($sql, DIB::$CONTAINERDATA[2], $params);
        if($rst === FALSE)
            return array('error','Warning, could not update database. Please contact the System Administrator');
        if(!isset($params[':placeIn'])) {
            $sql = "UPDATE `test_company`  SET `test_company`.`id` = " . ($orderNo + 5) . ", `parent_company_id` = :parentId
                    WHERE `test_company`.`id` = :nodeId";
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 9564, 'test_company', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getCaptions() {
    	return array('Id', 'Name', 'Chinese Name', 'Parent Company', 'Website', 'Parent Company Contact', 'Icon', 'Notes' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibtestGridTree",
