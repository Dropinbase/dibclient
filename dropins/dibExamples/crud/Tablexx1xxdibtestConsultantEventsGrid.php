<?php 
    Class Tablexx1xxdibtestConsultantEventsGrid { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "name"=>PDO::PARAM_STR, "employed_at_id"=>PDO::PARAM_INT, "mobile"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR, "address1"=>PDO::PARAM_STR, "address2"=>PDO::PARAM_STR, "city_town"=>PDO::PARAM_STR, "country"=>PDO::PARAM_STR, "longitude"=>PDO::PARAM_STR, "latitude"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "name"=>'none', "employed_at_id"=>'dropdown', "mobile"=>'none', "email"=>'none', "address1"=>'none', "address2"=>'none', "city_town"=>'none', "country"=>'none', "longitude"=>'none', "latitude"=>'none', "notes"=>'none');
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            PeffApp::load('dibExamples', 'ContainerEvents.php', 'components');
$t = new ContainerEvents();
$result = $t->afterReadMany('dibtestConsultantEventsGrid', $attributes, 'after readmany', $filterParams);
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id  are submitted to the server (ie they exist as (hidden) fields in container id 7149), or have defaults in pef_field.");
            ...
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 23321 are submitted to the server (ie they exist as fields in container id 7149)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(!empty($rst)) {
                if($makeUniqueValues)
    ...
    public function update($pkValues, $attributes) {
        ...
        try {    
            ...
            PeffApp::load('dibExamples', 'ContainerEvents.php', 'components');
$t = new ContainerEvents();
$result = $t->afterUpdate('dibtestConsultantEventsGrid', $attributes, 'after update', $recordOld, $validAttributes);
            if(isset($result[0]) && $result[0]==='cancel') return array('error', $result[1]);
            $crit = TRUE;
            if ($crit===TRUE) {
                // Insert audit trail record - first set unique_record
    ...
    public function delete($pkValues) {
        try {      
            ...
            PeffApp::load('dibExamples', 'ContainerEvents.php', 'components');
$t = new ContainerEvents();
$result = $t->beforeDelete('dibtestConsultantEventsGrid', $attributes, 'before delete');
            if(isset($result[0]) && $result[0]==='cancel') return array('error', $result[1]);
            dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);       
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for dibtestConsultantEventsGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibtestConsultantEventsGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestConsultantEventsGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibtestConsultantEventsGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
    ...
    public function getDefaults($createParams=null, $submissionData=array()) {
        try {
            ...
            PeffApp::load('dibExamples', 'ContainerEvents.php', 'components');
$t = new ContainerEvents();
$result = $t->getDefaults('dibtestConsultantEventsGrid', $attributes, 'get defaults');
            if(isset($result[0]) && $result[0]==='cancel') return array('error', $result[1]);
            return $attributes;
        }  catch (Exception $e) {
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 7149, 'test_consultant', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibtestConsultantEventsGrid",
