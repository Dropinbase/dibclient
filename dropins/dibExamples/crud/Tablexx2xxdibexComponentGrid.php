<?php 
    Class Tablexx2xxdibexComponentGrid { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "test_company_id"=>PDO::PARAM_INT, "test_company2_id"=>PDO::PARAM_INT, "varchar10_required"=>PDO::PARAM_STR, "has_default"=>PDO::PARAM_STR, "time_stamp"=>PDO::PARAM_STR, "unique_fld"=>PDO::PARAM_STR, "nvarchar80"=>PDO::PARAM_STR, "text_fld"=>PDO::PARAM_STR, "tinytext_fld"=>PDO::PARAM_STR, "mediumtext_fld"=>PDO::PARAM_STR, "longtext_fld"=>PDO::PARAM_STR, "bit_fld"=>PDO::PARAM_STR, "tinyint_fld"=>PDO::PARAM_BOOL, "smallint_fld"=>PDO::PARAM_INT, "int_fld"=>PDO::PARAM_INT, "bigint_fld"=>PDO::PARAM_INT, "float_fld"=>PDO::PARAM_STR, "double_fld"=>PDO::PARAM_STR, "decimal_fld"=>PDO::PARAM_STR, "date_fld"=>PDO::PARAM_STR, "time_fld"=>PDO::PARAM_STR, "datetime_fld"=>PDO::PARAM_STR, "year_fld"=>PDO::PARAM_INT, "enum_fld"=>PDO::PARAM_STR, "set_fld"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR, "url"=>PDO::PARAM_STR, "longitude"=>PDO::PARAM_STR, "lattitude"=>PDO::PARAM_STR, "file_fld"=>PDO::PARAM_STR, "image_fld"=>PDO::PARAM_STR, "document_fld"=>PDO::PARAM_STR, "expression_fld"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "test_company_id"=>'dropdown', "test_company2_id"=>'dropdown', "varchar10_required"=>'none', "has_default"=>'none', "time_stamp"=>'none', "unique_fld"=>'none', "nvarchar80"=>'none', "text_fld"=>'none', "tinytext_fld"=>'none', "mediumtext_fld"=>'none', "longtext_fld"=>'none', "bit_fld"=>'none', "tinyint_fld"=>'none', "smallint_fld"=>'none', "int_fld"=>'none', "bigint_fld"=>'none', "float_fld"=>'none', "double_fld"=>'none', "decimal_fld"=>'none', "date_fld"=>'none', "time_fld"=>'none', "datetime_fld"=>'none', "year_fld"=>'none', "enum_fld"=>'none', "set_fld"=>'none', "email"=>'none', "url"=>'none', "longitude"=>'none', "lattitude"=>'none', "file_fld"=>'none', "image_fld"=>'none', "document_fld"=>'none', "expression_fld"=>'none', "notes"=>'none');
...
$sql .= $criteria . $orderStr . $limit;               
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(!array_key_exists('unique_fld', $attributes)) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id  are submitted to the server (ie they exist as (hidden) fields in container id 7162), or have defaults in pef_field.");
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for dibexComponentGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexComponentGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if(empty($result)) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexComponentGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexComponentGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
    ...
    public function getDefaults($createParams=null, $submissionData=array()) {
        try {
            ...
            $attributes['time_stamp'] = CURRENT_TIMESTAMP;
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx2xxdibexComponentGrid",
