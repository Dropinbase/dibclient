<?php 
    Class Tablexx1xxdibexSubContainers { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_STR, "name"=>PDO::PARAM_STR, "parent_company_id"=>PDO::PARAM_INT, "parent_company_contact_id"=>PDO::PARAM_INT);
    public $storeType = array("id"=>'none', "name"=>'none', "parent_company_id"=>'dropdown', "parent_company_contact_id"=>'dropdown');
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $sql = "SELECT `test_company`.`id`,`test_company`.`name`,`test_company`.`parent_company_id`,`test_company`.`parent_company_contact_id` 
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `test_company`.`id` AS `Id`, `test_company`.`name` AS `Name` 
                , ^^CONCAT(`test_company1001`.`name`, ' (' , CAST(`test_company1001`.`id` AS CHAR), ')')^^ AS `parentCompanyId`, `test_consultant1002`.`name` AS `parentCompanyContactId`
            FROM `test_company` 
                LEFT JOIN `test_company` `test_company1001` ON `test_company`.`parent_company_id` = `test_company1001`.`id` 
                LEFT JOIN `test_consultant` `test_consultant1002` ON `test_company`.`parent_company_contact_id` = `test_consultant1002`.`id` 
                 ";
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "name"=>null, "parent_company_id"=>null, "parent_company_contact_id"=>null);
    ...
    public function create(&$attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(isset($attributes['parent_company_id'])) {            	
	            if(trim((string)($attributes['parent_company_id'])) !== '') {
		            if(!is_int((int)$attributes["parent_company_id"]) || !ctype_digit((string)abs($attributes["parent_company_id"])))
		                return array ('error',"The 'parentCompanyId (parent_company_id)' field must be an integer value.");
	            } 
	        }             
            ...
            if(isset($attributes['parent_company_contact_id'])) {            	
	            if(trim((string)($attributes['parent_company_contact_id'])) !== '') {
		            if(!is_int((int)$attributes["parent_company_contact_id"]) || !ctype_digit((string)abs($attributes["parent_company_contact_id"])))
		                return array ('error',"The 'parentCompanyContactId (parent_company_contact_id)' field must be an integer value.");
	            } 
	        }             
            //Check Unique Values for table option 23312
            $criteria = '1=1';
            if(!array_key_exists('name', $attributes)) {
                Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id  are submitted to the server (ie they exist as (hidden) fields in container id 7155), or have defaults in pef_field.");
            ...
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 23312 are submitted to the server (ie they exist as fields in container id 7155)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(dibMySqlPdo::count() > 0) {
                if($makeUniqueValues)
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `name`, `parent_company_id`, `parent_company_contact_id`
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for dibexSubContainers.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container dibexSubContainers records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if($result === FALSE || Database::count() === 0) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexSubContainers' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'dibexSubContainers' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 7155, 'test_company', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getCaptions() {
    	return array('Id', 'Name', 'parentCompanyId', 'parentCompanyContactId' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxdibexSubContainers",
					 'selectFields' => "`test_company`.`id`,`test_company`.`name`,`test_company`.`parent_company_id`,`test_company`.`parent_company_contact_id`",
