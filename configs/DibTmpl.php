...
class DIB {
	...
	public static $CODEUSE=2; // 0 = Always overwrite cache files (also occurs in 'development' mode); 
    ...
    public static $USERSPATH='~rootdir~files~dirsep~uploads'; // Physical path to user-file uploads folder (NOTE, keep outside webserver's reach for security reasons)
    											 // IMPORTANT: See https://www.owasp.org/index.php/Unrestricted_File_Upload
	// public static $USEPROXYPERMGROUP = TRUE; //Need to enable this later on
	...
	public static $VERIFY_IP=0; // check if we have to verify the IP
	public static $VERIFY_USER_AGENT=0; // check if we have to verify for the user agent
	public static $AUDITDBINDEX=1; // Determine where the audit trails will be stored
	public static $ERRORLOGINDEX=1;
	...
	public static $BASEURL='~securebaseurl~';
	...
	public static $VENDORPATH= '~systemdir~../../';
	...
	public static $CODEPATH; // Folder where current container's cache files are stored for the current user (incorporates user's Source Folder). Is set when container permissions are checked.
