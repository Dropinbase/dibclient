<?php 
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'clientForm_projectGrid') === 0) {
            ...
            if (array_key_exists('submitItemAlias_parent_client_pk',$filterParams)) {
                ...
                if(is_array($value) || $value === ':submitItemAlias_parent_client_pk')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_client_pk' for filter '' on projectGrid is missing from submitted values.");
        ...
        if(strpos($activeFilter, 'staffForm_projectGrid') === 0) {
            ...
            if (array_key_exists('submitItemAlias_parent_staff_pk',$filterParams)) {
                ...
                if(is_array($value) || $value === ':submitItemAlias_parent_staff_pk')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_staff_pk' for filter '' on projectGrid is missing from submitted values.");
                    $crit = '1 = 2'; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                else 
    ...
    public function getToolbarInfo($pkValues, $option, $activeFilter, $filterParams) {
        ...
        if(!empty($activeFilter)){  
            $crit = ''; 
            if($activeFilter === 'clientForm_projectGrid') {  
                $crit = "`project`.`client_id` = :submitItemAlias_parent_client_pk"; 
                if (array_key_exists('submitItemAlias_parent_client_pk',$filterParams)) {
                    $params[':submitItemAlias_parent_client_pk'] = $filterParams['submitItemAlias_parent_client_pk'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitItemAlias_parent_client_pk', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitItemAlias_parent_client_pk' for filter 'projectGrid' on projectGrid is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitItemAlias_parent_client_pk'] = $value;
                }
            } elseif($activeFilter === 'staffForm_projectGrid') {  
                $crit = "`project`.`project_leader_id` = :submitItemAlias_parent_staff_pk"; 
                if (array_key_exists('submitItemAlias_parent_staff_pk',$filterParams)) {
                    $params[':submitItemAlias_parent_staff_pk'] = $filterParams['submitItemAlias_parent_staff_pk'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitItemAlias_parent_staff_pk', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitItemAlias_parent_staff_pk' for filter 'projectGrid' on projectGrid is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitItemAlias_parent_staff_pk'] = $value;
                }
...
$sql .= $criteria . $orderStr . $limit;               
    ...
    public function create($attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 24575 are submitted to the server (ie they exist as fields in container id 9115)");
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for projectGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container projectGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if($result === FALSE || Database::count() === 0) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'projectGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'projectGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 9115, 'project', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxprojectGrid",
