<?php 
    Class Tablexx1xxcodeGrid { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'code';
    public $fieldType = array("code"=>PDO::PARAM_STR, "inhouse"=>PDO::PARAM_BOOL, "type"=>PDO::PARAM_STR, "classification"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR);
    public $storeType = array("code"=>'none', "inhouse"=>'none', "type"=>'none', "classification"=>'none', "notes"=>'none');
    ...
    public function getToolbarInfo($pkValues, $option, $activeFilter, $filterParams) {
        $params = array();
        $fieldType = array();
        if (!array_key_exists("code", $pkValues))
            return array ('error',"The primary key fields specified in the request are invalid.");        
        $params[":pk1"] = $pkValues["code"];
        $fieldType[":pk1"] = PDO::PARAM_STR;    
        $pkList = "`code`.`code`";
        $criteria = '';
        $order ='';
        if ($option === 'next')  { 
            $criteria = "WHERE `code` > :pk1";
            $order = "ORDER BY `code`.`code`";
        } elseif ($option === 'prev') {
            $order = "ORDER BY `code` DESC";
            $criteria = "WHERE `code` < :pk1";
        } elseif ($option === 'last') {
            $order = "ORDER BY `code`.`code` DESC";
            $params = array();
            $fieldType = array();
        } elseif ($option === 'total') {
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $params = array();
                if (!array_key_exists("code", $filterParams))
                    return array ('error',"The primary key fields specified in the request are invalid.");
                $params[":pk1"] = $filterParams["code"];
                $fieldType[":pk1"] = PDO::PARAM_STR;
                $criteria = "`code`.`code` = :pk1 ";
                $sql = "SELECT `code`.`code`,`code`.`inhouse`,`code`.`type`,`code`.`classification`,`code`.`notes` 
                         FROM `code`                  
                         WHERE $criteria";
                dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);
                ...
                if($page === 1 || $countMode==='all'){
                	$sql = "SELECT Count(*) AS totalcount FROM `code`  ";
                ...
                if ($criteria !== '') {
                 ...
                 ";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
	                    $sql = "SELECT Count(*) AS filteredCount FROM `code` $join  $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`code`.`' . $record['property'] . '` ' . $direction . ', ';
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `code`.`code` AS `Code`, `code`.`inhouse` AS `Inhouse`, `code`.`type` AS `Type`, `code`.`classification` AS `Classification`, `code`.`notes` AS `Notes` 
            FROM `code` 
                 ";
else
    $sql = "SELECT `code`.`code`,`code`.`inhouse`,`code`.`type`,`code`.`classification`,`code`.`notes` 
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("code"=>null, "type"=>null, "classification"=>null, "notes"=>null);
    ...
    public function create($attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(isset($attributes['code'])) {            	
	            if(trim((string)($attributes['code'])) !== '') {
	            	if(strlen($attributes["code"]) > 10)
		                return array ('error',"The 'Code (code)' field cannot contain more than 10 characters");
	            } 
	        }  else	            
                return array ('error',"The 'Code (code)' field is required. Please provide a value.");
	        // inhouse (boolean)
        	$attributes['inhouse'] = (!isset($attributes['inhouse']) || !in_array($attributes['inhouse'], array(1, True, '1', 'True'), TRUE)) ? 0 : 1;            	
	        // type (plain text)
            if(isset($attributes['type'])) {            	
	            if(trim((string)($attributes['type'])) !== '') {
	            	if(strlen($attributes["type"]) > 100)
		                return array ('error',"The 'Type (type)' field cannot contain more than 100 characters");
	            } 
	        }  else	            
                return array ('error',"The 'Type (type)' field is required. Please provide a value.");
	        // classification (plain text)
            if(isset($attributes['classification'])) {            	
	            if(trim((string)($attributes['classification'])) !== '') {
	            	if(strlen($attributes["classification"]) > 100)
		                return array ('error',"The 'Classification (classification)' field cannot contain more than 100 characters");
	            } 
	        }  else	            
                return array ('error',"The 'Classification (classification)' field is required. Please provide a value.");
	        // notes (plain text)
            ...
            if ($crit===TRUE) {
                ...
                // Get pk values
                $recordId='';
                $recordId = $attributes["code"];
                foreach ($attributes AS $fieldName => $newValue)
                    $this->auditInsert("create", $fieldName, null, $newValue, 4611, $recordId);
    ...
    public function update($pkValues, $attributes) {
        ...
        try {    
            ...
            // inhouse (boolean)	        
        	$attributes["inhouse"] = (in_array($attributes["inhouse"], array(1, True, '1', 'True'), TRUE)) ? 1 : 0;            	
            // type (plain text)	        
            if(isset($attributes['type'])) {            	
	            if(trim((string)($attributes['type'])) !== '') {
	            	if(strlen($attributes["type"]) > 100)
            ...
            if ($crit===TRUE) {
                ...
                foreach ($validAttributes AS $fieldName => $newValue) {
                    if ($newValue !== $recordOld[$fieldName])
                        $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 4611, $recordId);
                }
            } elseif (is_array($crit)) return $crit;
        } catch (Exception $e) {            
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) 
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 4611, $recordId);
                } elseif (is_array($crit)) return $crit;
                return true;
            }
        }  catch (Exception $e) {        
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `code`, `inhouse`, `type`, `classification`, `notes`
                    FROM `code` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for codeGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container codeGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if($result === FALSE || Database::count() === 0) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'codeGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'codeGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = 'UPDATE code SET `' . $args[0] . "` = :value WHERE $pkCrit";					
					PeffApp::$array['DuplicateRecords']['code*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 9136, 'code', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getCaptions() {
    	return array('Code', 'Inhouse', 'Type', 'Classification', 'Notes' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxcodeGrid",
					 'selectFields' => "`code`.`code`,`code`.`inhouse`,`code`.`type`,`code`.`classification`,`code`.`notes`",
