<?php 
    Class Tablexx1xxcompanyIsinForm { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "company_id"=>PDO::PARAM_INT, "isin"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "company_id"=>'dropdown', "isin"=>'none', "notes"=>'none');
    public $sqlFields = array();
    public $fkeyDisplay = array(
                 'company_id'=>"`company1001`.`name`", 
                 );
    ...
    private function parseFilterArray($filterParams=array(), &$params=array(), &$fieldType=array()) {
        ...
        foreach($filterParams as $record) {
            ...
            if(array_key_exists($field, $this->sqlFields))
            	...
            	$str .= '`company_isin`.`' . $field . '` = :pk' . $i . ' AND ';
            $params[':pk' . $i] = $value;
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'companyForm_companyIsinForm') === 0) {
            $crit = "`company_isin`.`company_id` = :submitItemAlias_parent_company_pk"; 
            if (array_key_exists('submitItemAlias_parent_company_pk',$filterParams)) {
                ...
                if(is_array($value) || $value === ':submitItemAlias_parent_company_pk')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_company_pk' for filter '' on companyIsinForm is missing from submitted values.");
    ...
    public function getToolbarInfo($pkValues, $option, $activeFilter, $filterParams) {
        ...
        $pkList = "`company_isin`.`id`";
        $criteria = '';
        $order ='';
        if ($option === 'next')  { 
            $criteria = "WHERE `id` > :pk1";
            $order = "ORDER BY `company_isin`.`id`";
        } elseif ($option === 'prev') {
            $order = "ORDER BY `id` DESC";
            $criteria = "WHERE `id` < :pk1";
        } elseif ($option === 'last') {
            $order = "ORDER BY `company_isin`.`id` DESC";
            $params = array();
            $fieldType = array();
        } elseif ($option === 'total') {
            ...
            $order = "ORDER BY `company_isin`.`id` ASC";
            $params = array();
            $fieldType = array();
        }
        // activeFilter                
        if(!empty($activeFilter)){  
            $crit = ''; 
            if($activeFilter === 'companyForm_companyIsinForm') {  
                $crit = "`company_isin`.`company_id` = :submitItemAlias_parent_company_pk"; 
                if (array_key_exists('submitItemAlias_parent_company_pk',$filterParams)) {
                    $params[':submitItemAlias_parent_company_pk'] = $filterParams['submitItemAlias_parent_company_pk'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitItemAlias_parent_company_pk', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitItemAlias_parent_company_pk' for filter 'companyIsinForm' on companyIsinForm is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitItemAlias_parent_company_pk'] = $value;
                }
...
$sql = "SELECT $pkList FROM `company_isin` $criteria $order LIMIT 1";
...
$sql = "SELECT `company_isin`.`id` 
        FROM `company_isin`
        $criteria
        ORDER BY `company_isin`.`id` 
        LIMIT " . ($position - 1) . ', 1'; 
        $rst = dibMySqlPdo::execute($sql, DIB::$CONTAINERDATA[2], $params, true);
        if(dibMySqlPdo::count() > 0)
            return $rst;
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`company_isin`.`id` = :pk1 ";
                $sql = "SELECT `company_isin`.`id`,`company_isin`.`company_id`,`company_isin`.`isin`,`company_isin`.`notes` 
                     , `company1001`.`name` AS `company_id_display_value`
                         FROM `company_isin`                  
                     LEFT JOIN `company` `company1001` ON `company_isin`.`company_id` = `company1001`.`id` 
                         WHERE $criteria";
                dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);
                ...
                if($page === 1 || $countMode==='all'){
                	$sql = "SELECT Count(*) AS totalcount FROM `company_isin`  ";
                ...
                if ($criteria !== '') {
                 ...
                 ";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
	                    $sql = "SELECT Count(*) AS filteredCount FROM `company_isin` $join  $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`company_isin`.`' . $record['property'] . '` ' . $direction . ', ';
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `company_isin`.`id` AS `Id`, `company_isin`.`isin` AS `Isin`, `company_isin`.`notes` AS `Notes` 
                     , `company1001`.`name` AS `Company`
            FROM `company_isin` 
                     LEFT JOIN `company` `company1001` ON `company_isin`.`company_id` = `company1001`.`id` 
                 ";
else
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "company_id"=>null, "isin"=>null, "notes"=>null);
    ...
    public function create($attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(isset($attributes['company_id'])) {            	
                ...
                return array ('error',"The 'Company (company_id)' field is required. Please provide a value.");
	        // isin (plain text)
            if(isset($attributes['isin'])) {            	
                ...
                return array ('error',"The 'Isin (isin)' field is required. Please provide a value.");
	        // notes (plain text)
            ...
            //Check Unique Values for company_isin
            if(!array_key_exists('company_id', $attributes)) $attributes['company_id'] = null;
            $criteria ="`company_id` = :fk1 AND ";
            if(!array_key_exists('isin', $attributes)) $attributes['isin'] = null;
            $criteria .="`isin` = :fk2 ";
			// ***TODO - can add user criteria here for conditional uniques...
            $sql = "SELECT `company_isin`.`id` AS pkv FROM `company_isin` WHERE $criteria";
            $paramsU = array(":fk1" => $attributes["company_id"], ":fk2" => $attributes["isin"]);
            $rst = dibMySqlPdo::execute($sql, $targetDatabaseId, $paramsU, true);
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 24609 are submitted to the server (ie they exist as fields in container id 9139)");
            ...
            if(dibMySqlPdo::count() > 0) {
                if($makeUniqueValues)
                    ...
                    $attributes['isin'] = SyncFunctions::cleanName($attributes['isin'],'company_isin', '');
                elseif(count($paramsU) > 1)
                    return array('error',"Add record cancelled. The combination of values in 'company_id,isin' needs to be unique. The record identified by '" . $rst['pkv'] . "' already contains these values.");
                else
                    return array('error',"Add record cancelled. The value in 'company_id,isin' needs to be unique. The record identified by '" . $rst['pkv'] . "' already contains this value.");
            }
            // All clear - perform the insert...
            $sql = "INSERT INTO `company_isin` (";
            $fieldList = '';
            $valueList = '';
            $fieldType = array();
            $i=0;
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue)
                    $this->auditInsert("create", $fieldName, null, $newValue, 4613, $recordId);
    ...
    public function update($pkValues, $attributes) {
        ...
        try {    
            ...
            $fieldType[":pk1"] = PDO::PARAM_INT;            
            // `id` = :pk0    
            $pkCrit = "`company_isin`.`id` = :pk1";                   
            $crit = $pkCrit;
            // Get record's existing (old) values
            $sql = "SELECT `company_isin`.* 
		            FROM `company_isin`
                     LEFT JOIN `company` `company1001` ON `company_isin`.`company_id` = `company1001`.`id` 
		                WHERE $crit";
            ...
            if ($crit===TRUE) {
                ...
                foreach ($validAttributes AS $fieldName => $newValue) {
                    if ($newValue !== $recordOld[$fieldName])
                        $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 4613, $recordId);
                }
            } elseif (is_array($crit)) return $crit;
        } catch (Exception $e) {            
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) 
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 4613, $recordId);
                } elseif (is_array($crit)) return $crit;
                return true;
            }
        }  catch (Exception $e) {        
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `company_id`, `isin`, `notes`
                    FROM `company_isin` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for companyIsinForm.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container companyIsinForm records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if($result === FALSE || Database::count() === 0) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'companyIsinForm' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'companyIsinForm' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = 'UPDATE company_isin SET `' . $args[0] . "` = :value WHERE $pkCrit";					
					PeffApp::$array['DuplicateRecords']['company_isin*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 9139, 'company_isin', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getCaptions() {
    	return array('Id', 'Company', 'Isin', 'Notes' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxcompanyIsinForm",
					 'selectFields' => "`company_isin`.`id`,`company_isin`.`company_id`,`company_isin`.`isin`,`company_isin`.`notes`",
