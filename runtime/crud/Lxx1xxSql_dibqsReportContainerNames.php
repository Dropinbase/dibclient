...
class Lxx1xxSql_dibqsReportContainerNames {    
	...
	*/
    public function getList($containerItemId, $page=1, $page_size, $query=null, $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        ...
        $permCrit = '';
        // Set sql parts                  
        $totalSql = "SELECT Count(*) AS totalcount FROM pef_container cn LEFT JOIN pef_table ct ON cn.pef_table_id = ct.id LEFT JOIN pef_sql cs ON cn.pef_sql_id = cs.id LEFT JOIN pef_dropin cd ON cn.pef_display_dropin_id = 3
  ";             
        $sql = "    cn.name AS `id` ,  CONCAT(cn.caption, ' (', cn.name, ')') AS id_display_value ";
        $from_clause = "pef_container cn LEFT JOIN pef_table ct ON cn.pef_table_id = ct.id LEFT JOIN pef_sql cs ON cn.pef_sql_id = cs.id LEFT JOIN pef_dropin cd ON cn.pef_display_dropin_id = 3";
        $display_field = "CONCAT(cn.caption, ' (', cn.name, ')')";
        $pkey = "cn.name";        
        $criteria = "cn.name LIKE '%test%Grid'";  
        // Process user filter
        if($query) {
