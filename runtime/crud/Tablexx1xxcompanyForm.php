<?php 
    Class Tablexx1xxcompanyForm { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "notes"=>PDO::PARAM_STR, "jse_code"=>PDO::PARAM_STR, "name"=>PDO::PARAM_STR, "short_name"=>PDO::PARAM_STR, "sector"=>PDO::PARAM_STR, "industry"=>PDO::PARAM_STR, "isin"=>PDO::PARAM_STR, "import_date"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "notes"=>'none', "jse_code"=>'none', "name"=>'none', "short_name"=>'none', "sector"=>'none', "industry"=>'none', "isin"=>'none', "import_date"=>'none');
    ...
    public function getToolbarInfo($pkValues, $option, $activeFilter, $filterParams) {
        ...
        if ($option === 'next')  { 
            ...
            $order = "ORDER BY `company`.`id`";
        } elseif ($option === 'prev') {
            $order = "ORDER BY `id` DESC";
            $criteria = "WHERE `id` < :pk1";
        } elseif ($option === 'last') {
            $order = "ORDER BY `company`.`id` DESC";
            $params = array();
            $fieldType = array();
        } elseif ($option === 'total') {
...
$sql = "SELECT $pkList FROM `company` $criteria $order LIMIT 1";
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`company`.`id` = :pk1 ";
                $sql = "SELECT `company`.`id`,`company`.`jse_code`,`company`.`name`,`company`.`short_name`,`company`.`sector`,`company`.`industry`,`company`.`isin`,`company`.`import_date`,`company`.`notes` 
                         FROM `company`                  
                         WHERE $criteria";
                dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);
                ...
                if($page === 1 || $countMode==='all'){
                	$sql = "SELECT Count(*) AS totalcount FROM `company`  ";
                ...
                if ($criteria !== '') {
                 ...
                 ";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
	                    $sql = "SELECT Count(*) AS filteredCount FROM `company` $join  $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`company`.`' . $record['property'] . '` ' . $direction . ', ';
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `company`.`id` AS `Id`, `company`.`jse_code` AS `Jse Code`, `company`.`name` AS `Name`, `company`.`short_name` AS `Short Name`, `company`.`sector` AS `Sector`, `company`.`industry` AS `Industry`, `company`.`isin` AS `Isin`, `company`.`import_date` AS `Import Date`, `company`.`notes` AS `Notes` 
            FROM `company` 
                 ";
else
    $sql = "SELECT `company`.`id`,`company`.`jse_code`,`company`.`name`,`company`.`short_name`,`company`.`sector`,`company`.`industry`,`company`.`isin`,`company`.`import_date`,`company`.`notes` 
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "jse_code"=>null, "name"=>null, "short_name"=>null, "sector"=>null, "industry"=>null, "isin"=>null, "import_date"=>null, "notes"=>null);
    ...
    public function create($attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            // Check Validation
	        // jse_code (plain text)
            if(isset($attributes['jse_code'])) {            	
	            if(trim((string)($attributes['jse_code'])) !== '') {
	            	if(strlen($attributes["jse_code"]) > 10)
		                return array ('error',"The 'Jse Code (jse_code)' field cannot contain more than 10 characters");
	            } 
	        }             
	        // name (plain text)
	        ...
	        // short_name (plain text)
            if(isset($attributes['short_name'])) {            	
	            if(trim((string)($attributes['short_name'])) !== '') {
	            	if(strlen($attributes["short_name"]) > 70)
		                return array ('error',"The 'Short Name (short_name)' field cannot contain more than 70 characters");
	            } 
	        }             
	        // sector (plain text)
            if(isset($attributes['sector'])) {            	
	            if(trim((string)($attributes['sector'])) !== '') {
	            	if(strlen($attributes["sector"]) > 50)
		                return array ('error',"The 'Sector (sector)' field cannot contain more than 50 characters");
	            } 
	        }             
	        // industry (plain text)
            if(isset($attributes['industry'])) {            	
	            if(trim((string)($attributes['industry'])) !== '') {
	            	if(strlen($attributes["industry"]) > 50)
		                return array ('error',"The 'Industry (industry)' field cannot contain more than 50 characters");
	            } 
	        }             
	        // isin (plain text)
            ...
            if(isset($attributes['import_date'])) {            	
                ...
                return array ('error',"The 'Import Date (import_date)' field is required. Please provide a value.");
	        // notes (plain text)
            ...
            $sql = "SELECT `company`.`id` AS pkv FROM `company` WHERE $criteria";
            $paramsU = array(":fk1" => $attributes["name"]);
            $rst = dibMySqlPdo::execute($sql, $targetDatabaseId, $paramsU, true);
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 24605 are submitted to the server (ie they exist as fields in container id 9137)");
            ...
            if(dibMySqlPdo::count() > 0) {
                if($makeUniqueValues)
                    ...
                    $attributes['name'] = SyncFunctions::cleanName($attributes['name'],'company', '');
            ...
            //Check Unique Values for jse_code
            if(!array_key_exists('jse_code', $attributes)) $attributes['jse_code'] = null;
            $criteria ="`jse_code` = :fk1 ";
			// ***TODO - can add user criteria here for conditional uniques...
            $sql = "SELECT `company`.`id` AS pkv FROM `company` WHERE $criteria";
            $paramsU = array(":fk1" => $attributes["jse_code"]);
            $rst = dibMySqlPdo::execute($sql, $targetDatabaseId, $paramsU, true);
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 24606 are submitted to the server (ie they exist as fields in container id 9137)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(dibMySqlPdo::count() > 0) {
                if($makeUniqueValues)
                    // Force unique values - for combinations, only enforce on first 
                    $attributes['jse_code'] = SyncFunctions::cleanName($attributes['jse_code'],'company', '');
                elseif(count($paramsU) > 1)
                    return array('error',"Add record cancelled. The combination of values in 'jse_code' needs to be unique. The record identified by '" . $rst['pkv'] . "' already contains these values.");
                else
                    return array('error',"Add record cancelled. The value in 'jse_code' needs to be unique. The record identified by '" . $rst['pkv'] . "' already contains this value.");
            }
            //Check Unique Values for isin
            if(!array_key_exists('isin', $attributes)) $attributes['isin'] = null;
            $criteria ="`isin` = :fk1 ";
			// ***TODO - can add user criteria here for conditional uniques...
            $sql = "SELECT `company`.`id` AS pkv FROM `company` WHERE $criteria";
            $paramsU = array(":fk1" => $attributes["isin"]);
            $rst = dibMySqlPdo::execute($sql, $targetDatabaseId, $paramsU, true);
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 24607 are submitted to the server (ie they exist as fields in container id 9137)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(dibMySqlPdo::count() > 0) {
                if($makeUniqueValues)
                    // Force unique values - for combinations, only enforce on first 
                    $attributes['isin'] = SyncFunctions::cleanName($attributes['isin'],'company', '');
                elseif(count($paramsU) > 1)
                    return array('error',"Add record cancelled. The combination of values in 'isin' needs to be unique. The record identified by '" . $rst['pkv'] . "' already contains these values.");
                else
                    return array('error',"Add record cancelled. The value in 'isin' needs to be unique. The record identified by '" . $rst['pkv'] . "' already contains this value.");
            }
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue)
                    $this->auditInsert("create", $fieldName, null, $newValue, 4612, $recordId);
            ...
            if ($crit===TRUE) {
                ...
                foreach ($validAttributes AS $fieldName => $newValue) {
                    if ($newValue !== $recordOld[$fieldName])
                        $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 4612, $recordId);
                }
            } elseif (is_array($crit)) return $crit;
        } catch (Exception $e) {            
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) 
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 4612, $recordId);
                } elseif (is_array($crit)) return $crit;
                return true;
            }
        }  catch (Exception $e) {        
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `jse_code`, `name`, `short_name`, `sector`, `industry`, `isin`, `import_date`, `notes`
                    FROM `company` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for companyForm.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container companyForm records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if($result === FALSE || Database::count() === 0) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'companyForm' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'companyForm' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = 'UPDATE company SET `' . $args[0] . "` = :value WHERE $pkCrit";					
					PeffApp::$array['DuplicateRecords']['company*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 9137, 'company', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getCaptions() {
    	return array('Id', 'Notes', 'Jse Code', 'Name', 'Short Name', 'Sector', 'Industry', 'Isin', 'Import Date' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxcompanyForm",
					 'selectFields' => "`company`.`id`,`company`.`jse_code`,`company`.`name`,`company`.`short_name`,`company`.`sector`,`company`.`industry`,`company`.`isin`,`company`.`import_date`,`company`.`notes`",
