<?php 
    Class Tablexx1xxstaffForm { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "join_date"=>PDO::PARAM_STR, "first_name"=>PDO::PARAM_STR, "pef_login_id"=>PDO::PARAM_INT, "last_name"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR, "staff_code"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR, "email"=>PDO::PARAM_STR, "mobile"=>PDO::PARAM_STR, "skype"=>PDO::PARAM_STR, "position"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "join_date"=>'none', "first_name"=>'none', "pef_login_id"=>'none', "last_name"=>'none', "notes"=>'none', "staff_code"=>'none', "updated"=>'none', "email"=>'none', "mobile"=>'none', "skype"=>'none', "position"=>'none');
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`staff`.`id` = :pk1 ";
                $sql = "SELECT `staff`.`id`,`staff`.`first_name`,`staff`.`last_name`,`staff`.`staff_code`,`staff`.`email`,`staff`.`mobile`,`staff`.`skype`,`staff`.`position`,`staff`.`join_date`,`staff`.`pef_login_id`,`staff`.`notes`,`staff`.`updated` 
                         FROM `staff`                  
                         WHERE $criteria";
                dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);
                ...
                if($page === 1 || $countMode==='all'){
                	$sql = "SELECT Count(*) AS totalcount FROM `staff`  ";
                ...
                if ($criteria !== '') {
                 ...
                 ";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
	                    $sql = "SELECT Count(*) AS filteredCount FROM `staff` $join  $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`staff`.`' . $record['property'] . '` ' . $direction . ', ';
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `staff`.`id` AS `Id`, `staff`.`first_name` AS `First Name`, `staff`.`last_name` AS `Last Name`, `staff`.`staff_code` AS `Staff Code`, `staff`.`email` AS `Email`, `staff`.`mobile` AS `Mobile`, `staff`.`skype` AS `Skype`, `staff`.`position` AS `Position`, `staff`.`join_date` AS `Join Date`, `staff`.`pef_login_id` AS `Pef Login`, `staff`.`notes` AS `Notes`, `staff`.`updated` AS `Updated` 
            FROM `staff` 
                 ";
else
    $sql = "SELECT `staff`.`id`,`staff`.`first_name`,`staff`.`last_name`,`staff`.`staff_code`,`staff`.`email`,`staff`.`mobile`,`staff`.`skype`,`staff`.`position`,`staff`.`join_date`,`staff`.`pef_login_id`,`staff`.`notes`,`staff`.`updated` 
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "first_name"=>null, "last_name"=>null, "staff_code"=>null, "email"=>null, "mobile"=>null, "skype"=>null, "position"=>null, "join_date"=>null, "pef_login_id"=>null, "notes"=>null);
    ...
    public function create($attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(isset($attributes['first_name'])) {            	
	            if(trim((string)($attributes['first_name'])) !== '') {
	            	if(strlen($attributes["first_name"]) > 50)
		                return array ('error',"The 'First Name (first_name)' field cannot contain more than 50 characters");
	            } 
	        }  else	            
            ...
            if(isset($attributes['last_name'])) {            	
	            if(trim((string)($attributes['last_name'])) !== '') {
	            	if(strlen($attributes["last_name"]) > 50)
		                return array ('error',"The 'Last Name (last_name)' field cannot contain more than 50 characters");
	            } 
	        }  else	            
                return array ('error',"The 'Last Name (last_name)' field is required. Please provide a value.");
	        // staff_code (plain text)
            if(isset($attributes['staff_code'])) {            	
	            if(trim((string)($attributes['staff_code'])) !== '') {
	            	if(strlen($attributes["staff_code"]) > 10)
		                return array ('error',"The 'Staff Code (staff_code)' field cannot contain more than 10 characters");
	            } 
	        }  else	            
                return array ('error',"The 'Staff Code (staff_code)' field is required. Please provide a value.");
	        // email (email)
            if(isset($attributes['email'])) {            	
	            if(trim((string)($attributes['email'])) !== '') {
	            	if(strlen($attributes["email"]) > 200)
		                return array ('error',"The 'Email (email)' field cannot contain more than 200 characters");
                ...
                return array ('error',"The 'Email (email)' field is required. Please provide a value.");
	        // mobile (plain text)
            if(isset($attributes['mobile'])) {            	
	            if(trim((string)($attributes['mobile'])) !== '') {
	            	if(strlen($attributes["mobile"]) > 30)
		                return array ('error',"The 'Mobile (mobile)' field cannot contain more than 30 characters");
	            } 
	        }  else	            
                return array ('error',"The 'Mobile (mobile)' field is required. Please provide a value.");
	        // skype (plain text)
            if(isset($attributes['skype'])) {            	
	            if(trim((string)($attributes['skype'])) !== '') {
	            	if(strlen($attributes["skype"]) > 40)
		                return array ('error',"The 'Skype (skype)' field cannot contain more than 40 characters");
	            } 
	        }             
	        // position (plain text)
	        ...
	        // join_date (date)
            if(isset($attributes['join_date'])) {            	
	            if(trim((string)($attributes['join_date'])) !== '') {
	        		if(!strtotime($attributes["join_date"]))
		                return array ('error',"The 'Join Date (join_date)' field must be a valid date.");
		            else
		                $attributes["join_date"] = date('Y-m-d', strtotime($attributes["join_date"]));
	            } 
	        }  else	            
                return array ('error',"The 'Join Date (join_date)' field is required. Please provide a value.");
	        // pef_login_id (integer)
            if(isset($attributes['pef_login_id'])) {            	
	            if(trim((string)($attributes['pef_login_id'])) !== '') {
		            if(!is_int((int)$attributes["pef_login_id"]) || !ctype_digit((string)abs($attributes["pef_login_id"])))
		                return array ('error',"The 'Pef Login (pef_login_id)' field must be an integer value.");
	            	if($attributes["pef_login_id"] < 0)
		                return array ('error',"The 'Pef Login (pef_login_id)' field value must be greater than or equal to 0");
	            } 
	        }  else	            
                return array ('error',"The 'Pef Login (pef_login_id)' field is required. Please provide a value.");
	        // notes (plain text)
            ...
            //Check Unique Values for first_name_last_name
            if(!array_key_exists('first_name', $attributes)) $attributes['first_name'] = null;
            $criteria ="`first_name` = :fk1 AND ";
            if(!array_key_exists('last_name', $attributes)) $attributes['last_name'] = null;
            $criteria .="`last_name` = :fk2 ";
			// ***TODO - can add user criteria here for conditional uniques...
            $sql = "SELECT `staff`.`id` AS pkv FROM `staff` WHERE $criteria";
            ...
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 24579 are submitted to the server (ie they exist as fields in container id 9118)");
            ...
            if(dibMySqlPdo::count() > 0) {
                if($makeUniqueValues)
                    ...
                    $attributes['first_name'] = SyncFunctions::cleanName($attributes['first_name'],'staff', '');
            ...
            $rst = dibMySqlPdo::execute($sql, $targetDatabaseId, $paramsU, true);
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 24580 are submitted to the server (ie they exist as fields in container id 9118)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(dibMySqlPdo::count() > 0) {
                if($makeUniqueValues)
                    // Force unique values - for combinations, only enforce on first 
                    $attributes['email'] = SyncFunctions::cleanName($attributes['email'],'staff', '');
                elseif(count($paramsU) > 1)
            ...
            //Check Unique Values for staff_code
            if(!array_key_exists('staff_code', $attributes)) $attributes['staff_code'] = null;
            $criteria ="`staff_code` = :fk1 ";
			// ***TODO - can add user criteria here for conditional uniques...
            $sql = "SELECT `staff`.`id` AS pkv FROM `staff` WHERE $criteria";
            $paramsU = array(":fk1" => $attributes["staff_code"]);
            $rst = dibMySqlPdo::execute($sql, $targetDatabaseId, $paramsU, true);
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 24581 are submitted to the server (ie they exist as fields in container id 9118)");
                return array('error',"Could not perform unique value validation. Please contact the System Administrator.");
            }
            if(dibMySqlPdo::count() > 0) {
                if($makeUniqueValues)
                    // Force unique values - for combinations, only enforce on first 
                    $attributes['staff_code'] = SyncFunctions::cleanName($attributes['staff_code'],'staff', '');
                elseif(count($paramsU) > 1)
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue)
                    $this->auditInsert("create", $fieldName, null, $newValue, 4597, $recordId);
            ...
            if ($crit===TRUE) {
                ...
                foreach ($validAttributes AS $fieldName => $newValue) {
                    if ($newValue !== $recordOld[$fieldName])
                        $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 4597, $recordId);
                }
            } elseif (is_array($crit)) return $crit;
        } catch (Exception $e) {            
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) 
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 4597, $recordId);
                } elseif (is_array($crit)) return $crit;
                return true;
            }
        }  catch (Exception $e) {        
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `first_name`, `last_name`, `staff_code`, `email`, `mobile`, `skype`, `position`, `join_date`, `pef_login_id`, `notes`, `updated`
                    FROM `staff` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for staffForm.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container staffForm records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if($result === FALSE || Database::count() === 0) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'staffForm' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'staffForm' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = 'UPDATE staff SET `' . $args[0] . "` = :value WHERE $pkCrit";					
					PeffApp::$array['DuplicateRecords']['staff*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 9118, 'staff', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getCaptions() {
    	return array('Id', 'Join Date', 'First Name', 'Pef Login', 'Last Name', 'Notes', 'Staff Code', 'Updated', 'Email', 'Mobile', 'Skype', 'Position' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxstaffForm",
					 'selectFields' => "`staff`.`id`,`staff`.`first_name`,`staff`.`last_name`,`staff`.`staff_code`,`staff`.`email`,`staff`.`mobile`,`staff`.`skype`,`staff`.`position`,`staff`.`join_date`,`staff`.`pef_login_id`,`staff`.`notes`,`staff`.`updated`",
