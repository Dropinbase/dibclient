<?php 
    ...
    public $fkeyDisplay = array(
                 'client_id'=>"`client1001`.`name`", 'project_leader_id'=>"`staff1002`.`staff_code`", 
                 );
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'clientForm_projectForm') === 0) {
            $crit = "`project`.`client_id` = :submitItemAlias_parent_client_pk"; 
            if (array_key_exists('submitItemAlias_parent_client_pk',$filterParams)) {
                ...
                if(is_array($value) || $value === ':submitItemAlias_parent_client_pk')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_client_pk' for filter '' on projectForm is missing from submitted values.");
        ...
        if(strpos($activeFilter, 'staffForm_projectForm') === 0) {
            $crit = "`project`.`project_leader_id` = :submitItemAlias_parent_staff_pk"; 
            if (array_key_exists('submitItemAlias_parent_staff_pk',$filterParams)) {
                ...
                if(is_array($value) || $value === ':submitItemAlias_parent_staff_pk')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_staff_pk' for filter '' on projectForm is missing from submitted values.");
                    $crit = '1 = 2'; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                else 
    ...
    public function getToolbarInfo($pkValues, $option, $activeFilter, $filterParams) {
        ...
        if ($option === 'next')  { 
            ...
            $order = "ORDER BY `project`.`id`";
        } elseif ($option === 'prev') {
            $order = "ORDER BY `id` DESC";
            $criteria = "WHERE `id` < :pk1";
        } elseif ($option === 'last') {
            $order = "ORDER BY `project`.`id` DESC";
            $params = array();
            $fieldType = array();
        } elseif ($option === 'total') {
        ...
        if(!empty($activeFilter)){  
            $crit = ''; 
            if($activeFilter === 'clientForm_projectForm') {  
                $crit = "`project`.`client_id` = :submitItemAlias_parent_client_pk"; 
                if (array_key_exists('submitItemAlias_parent_client_pk',$filterParams)) {
                    $params[':submitItemAlias_parent_client_pk'] = $filterParams['submitItemAlias_parent_client_pk'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitItemAlias_parent_client_pk', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitItemAlias_parent_client_pk' for filter 'projectForm' on projectForm is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitItemAlias_parent_client_pk'] = $value;
                }
            } elseif($activeFilter === 'staffForm_projectForm') {  
                $crit = "`project`.`project_leader_id` = :submitItemAlias_parent_staff_pk"; 
                if (array_key_exists('submitItemAlias_parent_staff_pk',$filterParams)) {
                    $params[':submitItemAlias_parent_staff_pk'] = $filterParams['submitItemAlias_parent_staff_pk'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitItemAlias_parent_staff_pk', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitItemAlias_parent_staff_pk' for filter 'projectForm' on projectForm is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitItemAlias_parent_staff_pk'] = $value;
                }
...
$sql = "SELECT $pkList FROM `project` $criteria $order LIMIT 1";
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`project`.`id` = :pk1 ";
                $sql = "SELECT `project`.`id`,`project`.`name`,`project`.`client_id`,`project`.`project_leader_id`,`project`.`notes`,`project`.`updated` 
                     , `client1001`.`name` AS `client_id_display_value`, `staff1002`.`staff_code` AS `project_leader_id_display_value`
                         FROM `project`                  
                     LEFT JOIN `client` `client1001` ON `project`.`client_id` = `client1001`.`id` 
                     LEFT JOIN `staff` `staff1002` ON `project`.`project_leader_id` = `staff1002`.`id` 
                         WHERE $criteria";
                dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);
                ...
                if($page === 1 || $countMode==='all'){
                	$sql = "SELECT Count(*) AS totalcount FROM `project`  ";
                ...
                if ($criteria !== '') {
                 ...
                 ";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
	                    $sql = "SELECT Count(*) AS filteredCount FROM `project` $join  $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`project`.`' . $record['property'] . '` ' . $direction . ', ';
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `project`.`id` AS `Id`, `project`.`name` AS `Name`, `project`.`notes` AS `Notes`, `project`.`updated` AS `Updated` 
                     , `client1001`.`name` AS `Client`, `staff1002`.`staff_code` AS `Project Leader`
            FROM `project` 
                     LEFT JOIN `client` `client1001` ON `project`.`client_id` = `client1001`.`id` 
                     LEFT JOIN `staff` `staff1002` ON `project`.`project_leader_id` = `staff1002`.`id` 
                 ";
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "name"=>null, "client_id"=>null, "project_leader_id"=>null, "notes"=>null);
    ...
    public function create($attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            if(isset($attributes['name'])) {            	
	            if(trim((string)($attributes['name'])) !== '') {
	            	if(strlen($attributes["name"]) > 50)
		                return array ('error',"The 'Name (name)' field cannot contain more than 50 characters");
	            } 
	        }  else	            
            ...
            if(isset($attributes['project_leader_id'])) {            	
	            if(trim((string)($attributes['project_leader_id'])) !== '') {
		            if(!is_int((int)$attributes["project_leader_id"]) || !ctype_digit((string)abs($attributes["project_leader_id"])))
		                return array ('error',"The 'Project Leader (project_leader_id)' field must be an integer value.");
	            	if($attributes["project_leader_id"] < 0)
		                return array ('error',"The 'Project Leader (project_leader_id)' field value must be greater than or equal to 0");
	            } 
	        }             
	        // notes (plain text)
            ...
            $sql = "SELECT `project`.`id` AS pkv FROM `project` WHERE $criteria";
            $paramsU = array(":fk1" => $attributes["name"]);
            $rst = dibMySqlPdo::execute($sql, $targetDatabaseId, $paramsU, true);
            if ($rst === FALSE) {
				Log::err("Unique value validation failed. Ensure that values for all fields that are involved in checking unique index of pef_table_option.id 24575 are submitted to the server (ie they exist as fields in container id 9114)");
            ...
            if(dibMySqlPdo::count() > 0) {
                if($makeUniqueValues)
                    ...
                    $attributes['name'] = SyncFunctions::cleanName($attributes['name'],'project', '');
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue)
                    $this->auditInsert("create", $fieldName, null, $newValue, 4595, $recordId);
            ...
            if ($crit===TRUE) {
                ...
                foreach ($validAttributes AS $fieldName => $newValue) {
                    if ($newValue !== $recordOld[$fieldName])
                        $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 4595, $recordId);
                }
            } elseif (is_array($crit)) return $crit;
        } catch (Exception $e) {            
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) 
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 4595, $recordId);
                } elseif (is_array($crit)) return $crit;
                return true;
            }
        }  catch (Exception $e) {        
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for projectForm.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container projectForm records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if($result === FALSE || Database::count() === 0) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'projectForm' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'projectForm' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = 'UPDATE project SET `' . $args[0] . "` = :value WHERE $pkCrit";					
					PeffApp::$array['DuplicateRecords']['project*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 9114, 'project', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxprojectForm",
					 'selectFields' => "`project`.`id`,`project`.`name`,`project`.`client_id`,`project`.`project_leader_id`,`project`.`notes`,`project`.`updated`",
