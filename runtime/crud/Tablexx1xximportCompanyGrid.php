<?php 
    Class Tablexx1xximportCompanyGrid { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "company_name"=>PDO::PARAM_STR, "isin"=>PDO::PARAM_STR, "company_short_name"=>PDO::PARAM_STR, "jse_alpha_code"=>PDO::PARAM_STR, "company_id"=>PDO::PARAM_INT, "dataset_id"=>PDO::PARAM_INT, "market_cap_id"=>PDO::PARAM_INT, "batch_no"=>PDO::PARAM_INT, "import_date"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "company_name"=>'none', "isin"=>'none', "company_short_name"=>'none', "jse_alpha_code"=>'none', "company_id"=>'dropdown', "dataset_id"=>'dropdown', "market_cap_id"=>'dropdown', "batch_no"=>'none', "import_date"=>'none');
    public $sqlFields = array();
    public $fkeyDisplay = array(
                 'company_id'=>"`company1001`.`name`", 'dataset_id'=>"`dataset1002`.`name`", 'market_cap_id'=>"CONCAT(`market_cap1003`.`jse_code`, '-', CAST(`market_cap1003`.`batch_no` AS CHAR))", 
                 );
    ...
    private function parseFilterArray($filterParams=array(), &$params=array(), &$fieldType=array()) {
        ...
        foreach($filterParams as $record) {
            ...
            if(array_key_exists($field, $this->sqlFields))
            	...
            	$str .= '`import_company`.`' . $field . '` = :pk' . $i . ' AND ';
            $params[':pk' . $i] = $value;
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'companyForm_importCompanyGrid') === 0) {
            $crit = "`import_company`.`company_id` = :submitItemAlias_parent_company_pk"; 
            if (array_key_exists('submitItemAlias_parent_company_pk',$filterParams)) {
                $params[':submitItemAlias_parent_company_pk'] = $filterParams['submitItemAlias_parent_company_pk'];
                ...
                // unset($filterParams["submitItemAlias_parent_company_pk"]);
            } else {
                $value = EvalCriteria::evalParam(':submitItemAlias_parent_company_pk', $filterParams);
                if(is_array($value) || $value === ':submitItemAlias_parent_company_pk')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_company_pk' for filter '' on importCompanyGrid is missing from submitted values.");
                    ...
                    $params[':submitItemAlias_parent_company_pk'] = $value;
            }
            $criteria .= " AND ($crit) ";
        }
        if(strpos($activeFilter, 'datasetForm_importCompanyGrid') === 0) {
            $crit = "`import_company`.`dataset_id` = :submitItemAlias_parent_dataset_pk"; 
            if (array_key_exists('submitItemAlias_parent_dataset_pk',$filterParams)) {
                $params[':submitItemAlias_parent_dataset_pk'] = $filterParams['submitItemAlias_parent_dataset_pk'];
                // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                // unset($filterParams["submitItemAlias_parent_dataset_pk"]);
            } else {
                $value = EvalCriteria::evalParam(':submitItemAlias_parent_dataset_pk', $filterParams);
                if(is_array($value) || $value === ':submitItemAlias_parent_dataset_pk')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_dataset_pk' for filter '' on importCompanyGrid is missing from submitted values.");
                    $crit = '1 = 2'; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                else 
                    $params[':submitItemAlias_parent_dataset_pk'] = $value;
            }
            $criteria .= " AND ($crit) ";
        }
        if(strpos($activeFilter, 'marketCapForm_importCompanyGrid') === 0) {
            $crit = "`import_company`.`market_cap_id` = :submitItemAlias_parent_market_cap_pk"; 
            if (array_key_exists('submitItemAlias_parent_market_cap_pk',$filterParams)) {
                $params[':submitItemAlias_parent_market_cap_pk'] = $filterParams['submitItemAlias_parent_market_cap_pk'];
                // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                // unset($filterParams["submitItemAlias_parent_market_cap_pk"]);
            } else {
                $value = EvalCriteria::evalParam(':submitItemAlias_parent_market_cap_pk', $filterParams);
                if(is_array($value) || $value === ':submitItemAlias_parent_market_cap_pk')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_market_cap_pk' for filter '' on importCompanyGrid is missing from submitted values.");
                    $crit = '1 = 2'; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                else 
                    $params[':submitItemAlias_parent_market_cap_pk'] = $value;
            }
            $criteria .= " AND ($crit) ";
        }
    ...
    public function getToolbarInfo($pkValues, $option, $activeFilter, $filterParams) {
        ...
        $pkList = "`import_company`.`id`";
        $criteria = '';
        $order ='';
        if ($option === 'next')  { 
            $criteria = "WHERE `id` > :pk1";
            $order = "ORDER BY `import_company`.`id`";
        } elseif ($option === 'prev') {
            $order = "ORDER BY `id` DESC";
            $criteria = "WHERE `id` < :pk1";
        } elseif ($option === 'last') {
            $order = "ORDER BY `import_company`.`id` DESC";
            $params = array();
            $fieldType = array();
        } elseif ($option === 'total') {
            ...
            $order = "ORDER BY `import_company`.`id` ASC";
            $params = array();
            $fieldType = array();
        }
        // activeFilter                
        if(!empty($activeFilter)){  
            $crit = ''; 
            if($activeFilter === 'companyForm_importCompanyGrid') {  
                $crit = "`import_company`.`company_id` = :submitItemAlias_parent_company_pk"; 
                if (array_key_exists('submitItemAlias_parent_company_pk',$filterParams)) {
                    $params[':submitItemAlias_parent_company_pk'] = $filterParams['submitItemAlias_parent_company_pk'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitItemAlias_parent_company_pk', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitItemAlias_parent_company_pk' for filter 'importCompanyGrid' on importCompanyGrid is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitItemAlias_parent_company_pk'] = $value;
                }
            } elseif($activeFilter === 'datasetForm_importCompanyGrid') {  
                $crit = "`import_company`.`dataset_id` = :submitItemAlias_parent_dataset_pk"; 
                if (array_key_exists('submitItemAlias_parent_dataset_pk',$filterParams)) {
                    $params[':submitItemAlias_parent_dataset_pk'] = $filterParams['submitItemAlias_parent_dataset_pk'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitItemAlias_parent_dataset_pk', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitItemAlias_parent_dataset_pk' for filter 'importCompanyGrid' on importCompanyGrid is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitItemAlias_parent_dataset_pk'] = $value;
                }
            } elseif($activeFilter === 'marketCapForm_importCompanyGrid') {  
                $crit = "`import_company`.`market_cap_id` = :submitItemAlias_parent_market_cap_pk"; 
                if (array_key_exists('submitItemAlias_parent_market_cap_pk',$filterParams)) {
                    $params[':submitItemAlias_parent_market_cap_pk'] = $filterParams['submitItemAlias_parent_market_cap_pk'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitItemAlias_parent_market_cap_pk', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitItemAlias_parent_market_cap_pk' for filter 'importCompanyGrid' on importCompanyGrid is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitItemAlias_parent_market_cap_pk'] = $value;
                }
...
$sql = "SELECT $pkList FROM `import_company` $criteria $order LIMIT 1";
...
$sql = "SELECT `import_company`.`id` 
        FROM `import_company`
        $criteria
        ORDER BY `import_company`.`id` 
        LIMIT " . ($position - 1) . ', 1'; 
        $rst = dibMySqlPdo::execute($sql, DIB::$CONTAINERDATA[2], $params, true);
        if(dibMySqlPdo::count() > 0)
            return $rst;
    ...
    public function parseGridFilter($gridFilter, &$params=array(), &$fieldType=array(), &$usedSqlField=FALSE) {
        ...
        foreach($gridFilter as $row) {
            ...
            if(array_key_exists($field, $this->sqlFields)) {
            	...
            	$fieldExpr = "`import_company`.`$field`";
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`import_company`.`id` = :pk1 ";
                $sql = "SELECT `import_company`.`id`,`import_company`.`company_name`,`import_company`.`isin`,`import_company`.`company_short_name`,`import_company`.`jse_alpha_code`,`import_company`.`company_id`,`import_company`.`dataset_id`,`import_company`.`market_cap_id`,`import_company`.`batch_no`,`import_company`.`import_date` 
                     , `company1001`.`name` AS `company_id_display_value`, `dataset1002`.`name` AS `dataset_id_display_value`, CONCAT(`market_cap1003`.`jse_code`, '-', CAST(`market_cap1003`.`batch_no` AS CHAR)) AS `market_cap_id_display_value`
                         FROM `import_company`                  
                     LEFT JOIN `company` `company1001` ON `import_company`.`company_id` = `company1001`.`id` 
                     LEFT JOIN `dataset` `dataset1002` ON `import_company`.`dataset_id` = `dataset1002`.`id` 
                     LEFT JOIN `market_cap` `market_cap1003` ON `import_company`.`market_cap_id` = `market_cap1003`.`id` 
                         WHERE $criteria";
                dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);
                ...
                if($page === 1 || $countMode==='all'){
                	$sql = "SELECT Count(*) AS totalcount FROM `import_company`  ";
                ...
                if ($criteria !== '') {
                 ...
                 ";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
	                    $sql = "SELECT Count(*) AS filteredCount FROM `import_company` $join  $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`import_company`.`' . $record['property'] . '` ' . $direction . ', ';
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `import_company`.`id` AS `Id`, `import_company`.`company_name` AS `Company Name`, `import_company`.`isin` AS `Isin`, `import_company`.`company_short_name` AS `Company Short Name`, `import_company`.`jse_alpha_code` AS `Jse Alpha Code`, `import_company`.`batch_no` AS `Batch No`, `import_company`.`import_date` AS `Import Date` 
                     , `company1001`.`name` AS `Company`, `dataset1002`.`name` AS `Dataset`, CONCAT(`market_cap1003`.`jse_code`, '-', CAST(`market_cap1003`.`batch_no` AS CHAR)) AS `Market Cap`
            FROM `import_company` 
                     LEFT JOIN `company` `company1001` ON `import_company`.`company_id` = `company1001`.`id` 
                     LEFT JOIN `dataset` `dataset1002` ON `import_company`.`dataset_id` = `dataset1002`.`id` 
                     LEFT JOIN `market_cap` `market_cap1003` ON `import_company`.`market_cap_id` = `market_cap1003`.`id` 
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "company_name"=>null, "isin"=>null, "company_short_name"=>null, "jse_alpha_code"=>null, "company_id"=>null, "dataset_id"=>null, "market_cap_id"=>null, "batch_no"=>null, "import_date"=>null);
    ...
    public function create($attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            // Check Validation
	        // company_name (plain text)
            if(isset($attributes['company_name'])) {            	
	            if(trim((string)($attributes['company_name'])) !== '') {
	            	if(strlen($attributes["company_name"]) > 80)
		                return array ('error',"The 'Company Name (company_name)' field cannot contain more than 80 characters");
	            } 
	        }             
	        // isin (plain text)
            if(isset($attributes['isin'])) {            	
	            if(trim((string)($attributes['isin'])) !== '') {
	            	if(strlen($attributes["isin"]) > 20)
		                return array ('error',"The 'Isin (isin)' field cannot contain more than 20 characters");
	            } 
	        }             
	        // company_short_name (plain text)
            if(isset($attributes['company_short_name'])) {            	
	            if(trim((string)($attributes['company_short_name'])) !== '') {
	            	if(strlen($attributes["company_short_name"]) > 80)
		                return array ('error',"The 'Company Short Name (company_short_name)' field cannot contain more than 80 characters");
	            } 
	        }             
	        // jse_alpha_code (plain text)
            if(isset($attributes['jse_alpha_code'])) {            	
	            if(trim((string)($attributes['jse_alpha_code'])) !== '') {
	            	if(strlen($attributes["jse_alpha_code"]) > 30)
		                return array ('error',"The 'Jse Alpha Code (jse_alpha_code)' field cannot contain more than 30 characters");
	            } 
	        }             
	        // company_id (integer)
            if(isset($attributes['company_id'])) {            	
	            if(trim((string)($attributes['company_id'])) !== '') {
		            if(!is_int((int)$attributes["company_id"]) || !ctype_digit((string)abs($attributes["company_id"])))
		                return array ('error',"The 'Company (company_id)' field must be an integer value.");
	            	if($attributes["company_id"] < 0)
		                return array ('error',"The 'Company (company_id)' field value must be greater than or equal to 0");
	            } 
	        }             
	        // dataset_id (integer)
            if(isset($attributes['dataset_id'])) {            	
	            if(trim((string)($attributes['dataset_id'])) !== '') {
		            if(!is_int((int)$attributes["dataset_id"]) || !ctype_digit((string)abs($attributes["dataset_id"])))
		                return array ('error',"The 'Dataset (dataset_id)' field must be an integer value.");
	            	if($attributes["dataset_id"] < 0)
		                return array ('error',"The 'Dataset (dataset_id)' field value must be greater than or equal to 0");
	            } 
	        }             
	        // market_cap_id (integer)
            if(isset($attributes['market_cap_id'])) {            	
	            if(trim((string)($attributes['market_cap_id'])) !== '') {
		            if(!is_int((int)$attributes["market_cap_id"]) || !ctype_digit((string)abs($attributes["market_cap_id"])))
		                return array ('error',"The 'Market Cap (market_cap_id)' field must be an integer value.");
	            	if($attributes["market_cap_id"] < 0)
		                return array ('error',"The 'Market Cap (market_cap_id)' field value must be greater than or equal to 0");
	            } 
	        }             
	        // batch_no (integer)
            if(isset($attributes['batch_no'])) {            	
	            if(trim((string)($attributes['batch_no'])) !== '') {
		            if(!is_int((int)$attributes["batch_no"]) || !ctype_digit((string)abs($attributes["batch_no"])))
		                return array ('error',"The 'Batch No (batch_no)' field must be an integer value.");
	            } 
	        }             
	        // import_date (datetime)
            if(isset($attributes['import_date'])) {            	
	            if(trim((string)($attributes['import_date'])) !== '') {
	        		if(!strtotime($attributes["import_date"]))
		                return array ('error',"The 'Import Date (import_date)' field must be a valid date-time.");
		            else
		                $attributes["import_date"] = date('Y-m-d H:i:s', strtotime($attributes["import_date"]));
	            } 
	        }             
            // All clear - perform the insert...
            $sql = "INSERT INTO `import_company` (";
            $fieldList = '';
            $valueList = '';
            $fieldType = array();
            $i=0;
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue)
                    $this->auditInsert("create", $fieldName, null, $newValue, 4617, $recordId);
    ...
    public function update($pkValues, $attributes) {
        ...
        try {    
            ...
            $fieldType[":pk1"] = PDO::PARAM_INT;            
            // `id` = :pk0    
            $pkCrit = "`import_company`.`id` = :pk1";                   
            $crit = $pkCrit;
            // Get record's existing (old) values
            $sql = "SELECT `import_company`.* 
		            FROM `import_company`
                     LEFT JOIN `company` `company1001` ON `import_company`.`company_id` = `company1001`.`id` 
                     LEFT JOIN `dataset` `dataset1002` ON `import_company`.`dataset_id` = `dataset1002`.`id` 
                     LEFT JOIN `market_cap` `market_cap1003` ON `import_company`.`market_cap_id` = `market_cap1003`.`id` 
            ...
            if ($crit===TRUE) {
                ...
                foreach ($validAttributes AS $fieldName => $newValue) {
                    if ($newValue !== $recordOld[$fieldName])
                        $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 4617, $recordId);
                }
            } elseif (is_array($crit)) return $crit;
        } catch (Exception $e) {            
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) 
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 4617, $recordId);
                } elseif (is_array($crit)) return $crit;
                return true;
            }
        }  catch (Exception $e) {        
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `company_name`, `isin`, `company_short_name`, `jse_alpha_code`, `company_id`, `dataset_id`, `market_cap_id`, `batch_no`, `import_date`
                    FROM `import_company` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for importCompanyGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container importCompanyGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if($result === FALSE || Database::count() === 0) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'importCompanyGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'importCompanyGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = 'UPDATE import_company SET `' . $args[0] . "` = :value WHERE $pkCrit";					
					PeffApp::$array['DuplicateRecords']['import_company*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 9144, 'import_company', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getCaptions() {
    	return array('Id', 'Company Name', 'Isin', 'Company Short Name', 'Jse Alpha Code', 'Company', 'Dataset', 'Market Cap', 'Batch No', 'Import Date' );
    }
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xximportCompanyGrid",
					 'selectFields' => "`import_company`.`id`,`import_company`.`company_name`,`import_company`.`isin`,`import_company`.`company_short_name`,`import_company`.`jse_alpha_code`,`import_company`.`company_id`,`import_company`.`dataset_id`,`import_company`.`market_cap_id`,`import_company`.`batch_no`,`import_company`.`import_date`",
