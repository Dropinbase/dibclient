<?php 
    Class Tablexx1xxstaffProjectGrid { 
    public $count = 0;
    public $newPkValue = null; // for INSERTS it will contain lastInsertId
    public $pkeys = 'id';
    public $fieldType = array("id"=>PDO::PARAM_INT, "staff_id"=>PDO::PARAM_INT, "project_id"=>PDO::PARAM_INT, "date_from"=>PDO::PARAM_STR, "date_to"=>PDO::PARAM_STR, "notes"=>PDO::PARAM_STR, "updated"=>PDO::PARAM_STR);
    public $storeType = array("id"=>'none', "staff_id"=>'dropdown', "project_id"=>'dropdown', "date_from"=>'none', "date_to"=>'none', "notes"=>'none', "updated"=>'none');
    public $sqlFields = array();
    public $fkeyDisplay = array(
                 'staff_id'=>"`staff1001`.`staff_code`", 'project_id'=>"`project1002`.`name`", 
                 );
    ...
    private function parseFilterArray($filterParams=array(), &$params=array(), &$fieldType=array()) {
        ...
        foreach($filterParams as $record) {
            ...
            if(array_key_exists($field, $this->sqlFields))
            	...
            	$str .= '`staff_project`.`' . $field . '` = :pk' . $i . ' AND ';
            $params[':pk' . $i] = $value;
    ...
    public function parseFilter($activeFilter, &$params=array(), &$filterParams=array()) {
		$criteria = ''; 
        if(strpos($activeFilter, 'projectForm_staffProjectGrid') === 0) {
            $crit = "`staff_project`.`project_id` = :submitItemAlias_parent_project_pk"; 
            if (array_key_exists('submitItemAlias_parent_project_pk',$filterParams)) {
                $params[':submitItemAlias_parent_project_pk'] = $filterParams['submitItemAlias_parent_project_pk'];
                ...
                // unset($filterParams["submitItemAlias_parent_project_pk"]);
            } else {
                $value = EvalCriteria::evalParam(':submitItemAlias_parent_project_pk', $filterParams);
                if(is_array($value) || $value === ':submitItemAlias_parent_project_pk')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_project_pk' for filter '' on staffProjectGrid is missing from submitted values.");
                    ...
                    $params[':submitItemAlias_parent_project_pk'] = $value;
            }
            $criteria .= " AND ($crit) ";
        }
        if(strpos($activeFilter, 'staffForm_staffProjectGrid') === 0) {
            $crit = "`staff_project`.`staff_id` = :submitItemAlias_parent_staff_pk"; 
            if (array_key_exists('submitItemAlias_parent_staff_pk',$filterParams)) {
                $params[':submitItemAlias_parent_staff_pk'] = $filterParams['submitItemAlias_parent_staff_pk'];
                // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                // unset($filterParams["submitItemAlias_parent_staff_pk"]);
            } else {
                $value = EvalCriteria::evalParam(':submitItemAlias_parent_staff_pk', $filterParams);
                if(is_array($value) || $value === ':submitItemAlias_parent_staff_pk')
                    // ***TODO LogERROR!
                    //return array('error',"Error! The filter parameter 'submitItemAlias_parent_staff_pk' for filter '' on staffProjectGrid is missing from submitted values.");
                    $crit = '1 = 2'; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                else 
                    $params[':submitItemAlias_parent_staff_pk'] = $value;
            }
            $criteria .= " AND ($crit) ";
        }
    ...
    public function getToolbarInfo($pkValues, $option, $activeFilter, $filterParams) {
        ...
        $pkList = "`staff_project`.`id`";
        $criteria = '';
        $order ='';
        if ($option === 'next')  { 
            $criteria = "WHERE `id` > :pk1";
            $order = "ORDER BY `staff_project`.`id`";
        } elseif ($option === 'prev') {
            $order = "ORDER BY `id` DESC";
            $criteria = "WHERE `id` < :pk1";
        } elseif ($option === 'last') {
            $order = "ORDER BY `staff_project`.`id` DESC";
            $params = array();
            $fieldType = array();
        } elseif ($option === 'total') {
            ...
            $order = "ORDER BY `staff_project`.`id` ASC";
            $params = array();
            $fieldType = array();
        }
        // activeFilter                
        if(!empty($activeFilter)){  
            $crit = ''; 
            if($activeFilter === 'projectForm_staffProjectGrid') {  
                $crit = "`staff_project`.`project_id` = :submitItemAlias_parent_project_pk"; 
                if (array_key_exists('submitItemAlias_parent_project_pk',$filterParams)) {
                    $params[':submitItemAlias_parent_project_pk'] = $filterParams['submitItemAlias_parent_project_pk'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitItemAlias_parent_project_pk', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitItemAlias_parent_project_pk' for filter 'staffProjectGrid' on staffProjectGrid is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitItemAlias_parent_project_pk'] = $value;
                }
            } elseif($activeFilter === 'staffForm_staffProjectGrid') {  
                $crit = "`staff_project`.`staff_id` = :submitItemAlias_parent_staff_pk"; 
                if (array_key_exists('submitItemAlias_parent_staff_pk',$filterParams)) {
                    $params[':submitItemAlias_parent_staff_pk'] = $filterParams['submitItemAlias_parent_staff_pk'];
                    // Remove from array so that Related Records' parseFilterArray can add any other params, e.g. if Related Records and activeFilter both apply
                    ...
                    $value = EvalCriteria::evalParam(':submitItemAlias_parent_staff_pk', $filterParams);
                    if(is_array($value))
                        //return array('error',"Error! The filter parameter 'submitItemAlias_parent_staff_pk' for filter 'staffProjectGrid' on staffProjectGrid is missing from submitted values.");
                        $crit = ' 1=2 '; // We're returning no records since if eg submitCheckedItems is used and there are no checked records then this error will occur.
                    else 
                        $params[':submitItemAlias_parent_staff_pk'] = $value;
                }
...
$sql = "SELECT $pkList FROM `staff_project` $criteria $order LIMIT 1";
...
$sql = "SELECT `staff_project`.`id` 
        FROM `staff_project`
        $criteria
        ORDER BY `staff_project`.`id` 
        LIMIT " . ($position - 1) . ', 1'; 
        $rst = dibMySqlPdo::execute($sql, DIB::$CONTAINERDATA[2], $params, true);
        if(dibMySqlPdo::count() > 0)
            return $rst;
    ...
    public function read ($page=1, $page_size=40, $order=array(), $readType='gridlist', $gridFilter=array(), $activeFilter=null, $filterParams=array(), $phpFilter=null, $phpFilterParams=array(), $group=null, $node=null, $action=null, $actionData=null, $countMode='all') {
        ...
        try {
            if ($order === '*readByPk*') {
                ...
                $fieldType[":pk1"] = PDO::PARAM_INT;
                $criteria = "`staff_project`.`id` = :pk1 ";
                $sql = "SELECT `staff_project`.`id`,`staff_project`.`staff_id`,`staff_project`.`project_id`,`staff_project`.`date_from`,`staff_project`.`date_to`,`staff_project`.`notes`,`staff_project`.`updated` 
                     , `staff1001`.`staff_code` AS `staff_id_display_value`, `project1002`.`name` AS `project_id_display_value`
                         FROM `staff_project`                  
                     LEFT JOIN `staff` `staff1001` ON `staff_project`.`staff_id` = `staff1001`.`id` 
                     LEFT JOIN `project` `project1002` ON `staff_project`.`project_id` = `project1002`.`id` 
                         WHERE $criteria";
                dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);
                ...
                if($page === 1 || $countMode==='all'){
                	$sql = "SELECT Count(*) AS totalcount FROM `staff_project`  ";
                ...
                if ($criteria !== '') {
                 ...
                 ";
                 		...
                 		$join = '';
                    if($page === 1 || $countMode==='all'){
	                    $sql = "SELECT Count(*) AS filteredCount FROM `staff_project` $join  $criteria";
	                    dibMySqlPdo::setParamsType($fieldType, DIB::$CONTAINERDATA[2]);                          
                ...
                if($order) {
                    ...
                    foreach($order as $key => $record) {
                        ...
                        elseif(array_key_exists($record['property'], $this->fkeyDisplay))
                        	...
                        	$orderStr .= '`staff_project`.`' . $record['property'] . '` ' . $direction . ', ';
...
if($readType === 'exportlist')
    $sql = "SELECT 
                `staff_project`.`id` AS `Id`, `staff_project`.`date_from` AS `Date From`, `staff_project`.`date_to` AS `Date To`, `staff_project`.`notes` AS `Notes`, `staff_project`.`updated` AS `Updated` 
                     , `staff1001`.`staff_code` AS `Staff`, `project1002`.`name` AS `Project`
            FROM `staff_project` 
                     LEFT JOIN `staff` `staff1001` ON `staff_project`.`staff_id` = `staff1001`.`id` 
                     LEFT JOIN `project` `project1002` ON `staff_project`.`project_id` = `project1002`.`id` 
                 ";
...
$sql .= $criteria . $orderStr . $limit;               
            ...
            if ($action === 'add') {
                ...
                $blankRecord = array("id"=>null, "staff_id"=>null, "project_id"=>null, "date_from"=>null, "date_to"=>null, "notes"=>null);
    ...
    public function create($attributes, $makeUniqueValues=FALSE, $targetDatabaseId=NULL) {        
        ...
        try { 
            ...
            // Check Validation
	        // staff_id (integer)
            if(isset($attributes['staff_id'])) {            	
	            if(trim((string)($attributes['staff_id'])) !== '') {
		            if(!is_int((int)$attributes["staff_id"]) || !ctype_digit((string)abs($attributes["staff_id"])))
		                return array ('error',"The 'Staff (staff_id)' field must be an integer value.");
	            	if($attributes["staff_id"] < 0)
		                return array ('error',"The 'Staff (staff_id)' field value must be greater than or equal to 0");
	            } 
	        }  else	            
                return array ('error',"The 'Staff (staff_id)' field is required. Please provide a value.");
	        // project_id (integer)
            if(isset($attributes['project_id'])) {            	
	            if(trim((string)($attributes['project_id'])) !== '') {
		            if(!is_int((int)$attributes["project_id"]) || !ctype_digit((string)abs($attributes["project_id"])))
		                return array ('error',"The 'Project (project_id)' field must be an integer value.");
	            	if($attributes["project_id"] < 0)
		                return array ('error',"The 'Project (project_id)' field value must be greater than or equal to 0");
	            } 
	        }  else	            
                return array ('error',"The 'Project (project_id)' field is required. Please provide a value.");
	        // date_from (date)
            if(isset($attributes['date_from'])) {            	
	            if(trim((string)($attributes['date_from'])) !== '') {
	        		if(!strtotime($attributes["date_from"]))
		                return array ('error',"The 'Date From (date_from)' field must be a valid date.");
		            else
		                $attributes["date_from"] = date('Y-m-d', strtotime($attributes["date_from"]));
	            } 
	        }  else	            
                return array ('error',"The 'Date From (date_from)' field is required. Please provide a value.");
	        // date_to (date)
            if(isset($attributes['date_to'])) {            	
	            if(trim((string)($attributes['date_to'])) !== '') {
	        		if(!strtotime($attributes["date_to"]))
		                return array ('error',"The 'Date To (date_to)' field must be a valid date.");
		            else
		                $attributes["date_to"] = date('Y-m-d', strtotime($attributes["date_to"]));
	            } 
	        }             
	        // notes (plain text)
            if(isset($attributes['notes'])) {            	
	            if(trim((string)($attributes['notes'])) !== '') {
	            	if(strlen($attributes["notes"]) > 50)
		                return array ('error',"The 'Notes (notes)' field cannot contain more than 50 characters");
	            } 
	        }             
	        // updated (datetime)
            ...
            if ($crit===TRUE) {
                ...
                foreach ($attributes AS $fieldName => $newValue)
                    $this->auditInsert("create", $fieldName, null, $newValue, 4598, $recordId);
        ...
        try {    
            ...
            if(isset($attributes['id'])) {            	
	            if(trim((string)($attributes['id'])) !== '') {
	            	...
	            	if($attributes["id"] < 0)
		                return array ('error',"The 'Id (id)' field value must be greater than or equal to 0");
	            } 
	        }             
            // staff_id (integer)	        
            if(isset($attributes['staff_id'])) {            	
            ...
            $fieldType[":pk1"] = PDO::PARAM_INT;            
            // `id` = :pk0    
            $pkCrit = "`staff_project`.`id` = :pk1";                   
            $crit = $pkCrit;
            // Get record's existing (old) values
            $sql = "SELECT `staff_project`.* 
		            FROM `staff_project`
                     LEFT JOIN `staff` `staff1001` ON `staff_project`.`staff_id` = `staff1001`.`id` 
                     LEFT JOIN `project` `project1002` ON `staff_project`.`project_id` = `project1002`.`id` 
		                WHERE $crit";
            ...
            if ($crit===TRUE) {
                ...
                foreach ($validAttributes AS $fieldName => $newValue) {
                    if ($newValue !== $recordOld[$fieldName])
                        $this->auditInsert("update", $fieldName, $recordOld[$fieldName], $newValue, 4598, $recordId);
                }
            } elseif (is_array($crit)) return $crit;
        } catch (Exception $e) {            
            ...
            if (dibMySqlPdo::count() > 0) {
                ...
                if ($crit===TRUE) {
                    ...
                    foreach ($attributes AS $fieldName => $oldValue) 
                        $this->auditInsert("delete", $fieldName, $oldValue, NULL, 4598, $recordId);
                } elseif (is_array($crit)) return $crit;
                return true;
            }
        }  catch (Exception $e) {        
    ...
    public function duplicate($pkValues, $setValues=array(), $targetDatabaseId=null) {        
        ...
        try {
            ...
            $sql = "SELECT 1 AS to_prohibit_error_if_none, `id`, `staff_id`, `project_id`, `date_from`, `date_to`, `notes`, `updated`
                    FROM `staff_project` WHERE $pkCrit";
            ...
            if($record === FALSE || count($record) < 1){
				Log::err(count($record) . "SQL error while fetching values to duplicate for staffProjectGrid.\r\nSQL: $sql\r\nPARAMS:" . json_encode($params) . "\r\nERROR:" . Database::lastErrorAdminMsg());
                ...
                if (array_key_exists($field, $record)) {                	
					...
					elseif(!is_null($record[$field])) { 
						...
						if($result === FALSE || count($result) < 1) {
							Log::err("Unusual Sql error... No result returned in SOURCE db when attempting to find original values for a LOOKUP query while duplicating container staffProjectGrid records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[0] . "\r\n\PARAMS: " . json_encode($args));
							...
							if($result === FALSE || Database::count() === 0) {
								...
								if(isset($value[2]) && $value[2]==='create') {
									...
									if(isset($result[0]) && $result[0]==='error') {	
										Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'staffProjectGrid' records using the 'create' directive. Note, unless the create code succeeds, the whole operation will fail:\r\nLAST SQL ERROR:" . Database::lastErrorAdminMsg() . "\r\nPARAMS: " . json_encode($value[4]));
		    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
		    						}
		    						$result = array('id'=>$result);
		    					} else {
		    						Log::err("Sql error, or no result returned when attempting to run a LOOKUP query against database id $targetDatabaseId while duplicating container 'staffProjectGrid' records. Note, unless this query returns a value, the code will not work.\r\nSQL: " . $value[1] . "\r\n\PARAMS: " . json_encode($args));
	    							return array('error', "Configuration error found while duplicating records. Please contact the System Administrator.");
	    						}
							}							
            ...
            elseif ($delayArray) {
				...
				foreach($delayArray as $args) {// store table name, field name, pkey value, sql statements x 2 and sql params x 2
					$sql = 'UPDATE staff_project SET `' . $args[0] . "` = :value WHERE $pkCrit";					
					PeffApp::$array['DuplicateRecords']['staff_project*'.$args[0].'*'.$value] = array($args[1], $args[2], $sql, $params);
				}
				return $value;
			} else
    ...
    protected function auditInsert($crudType, $fieldName, $oldValue, $newValue, $tableId, $recordId) {
        $sql = "INSERT INTO `pef_audit_trail` 
             ...
             VALUES ('$crudType', $tableId, 9121, 'staff_project', :recordId, :dateTime, :ipAddress, :fieldName, :oldValue, :newValue, :loginId, :username, :unique_record)";
    ...
    public function getSqlParts() {
		return array('model' => 'Table',
					 'containerName' => "Tablexx1xxstaffProjectGrid",
					 'selectFields' => "`staff_project`.`id`,`staff_project`.`staff_id`,`staff_project`.`project_id`,`staff_project`.`date_from`,`staff_project`.`date_to`,`staff_project`.`notes`,`staff_project`.`updated`",
