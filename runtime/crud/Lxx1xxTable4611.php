	...
	*/
    public function getList($containerItemId, $page=1, $page_size=30, $query=null, $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        try {
            ...
            $params = array();
            $sql = "`code`.`code` AS id, CONCAT(`code`.`code`, '-', `code`.`type`) AS `id_display_value`";
            $from_clause = "`code`";
            $display_field = "CONCAT(`code`.`code`, '-', `code`.`type`)";
            $pkey = "`code`.`code`";
            $order_by = ($phpFilter === '') ? " ORDER BY CONCAT(`code`.`code`, '-', `code`.`type`)" : '';
            $totalSql = "SELECT count(*) AS totalcount FROM `code` ";
            // Add sql to show only used values (especially for filter dropdowns) - does not work for mssql...
            if($showUsedOnly) { 
            	$itemTables = array(211177=>"`beneficiary`.`code`", 211196=>"`beneficiary`.`code`", 211384=>"`import_strate`.`code`", 211415=>"`import_strate`.`code`", 211466=>"`inet_bfa`.`code`", 211496=>"`inet_bfa`.`code`", 211693=>"`share`.`code`", 211713=>"`share`.`code`", 211761=>"`xxxben_code`.`code`", 211767=>"`xxxben_code`.`code`");
            	if(isset($itemTables[$containerItemId])) {
            		$foreignParts = explode('.', $itemTables[$containerItemId]);
					$addSql = " INNER JOIN " . $foreignParts[0] . " dib___F ON dib___F." . $foreignParts[1] . " = `code`.`code` ";
            ...
            if($pageNoFromValue) {
            	...
            	$rst = dibMySqlPdo::execute("SELECT CONCAT(`code`.`code`, '-', `code`.`type`) AS dib__Display FROM `code` WHERE `code`.`code` = :pkey", DIB::$ITEMLISTDATA[3], array(':pkey'=>$pageNoFromValue), TRUE);
	            if($rst === FALSE)
	                return array('error', "Error! Could not read dropdown data information. Please contact the System Administrator");
				if($criteria) 
					$criteria = " WHERE $criteria AND CONCAT(`code`.`code`, '-', `code`.`type`) <= :dib__Display $order_by";
				else 
					$criteria = " WHERE CONCAT(`code`.`code`, '-', `code`.`type`) <= :dib__Display $order_by";
