	...
	*/
    public function getList($containerItemId, $page=1, $page_size=30, $query=null, $activeFilter=null, $filterParams=null, $phpFilter='', $phpFilterParams=array(), $pageNoFromValue=null, $showUsedOnly = false) {
        try {
            ...
            $params = array();
            $sql = "`client`.`id` AS id, `client`.`name` AS `id_display_value`";
            $from_clause = "`client`";
            $display_field = "`client`.`name`";
            $pkey = "`client`.`id`";
            $order_by = ($phpFilter === '') ? " ORDER BY `client`.`name`" : '';
            $totalSql = "SELECT count(*) AS totalcount FROM `client` ";
            // Add sql to show only used values (especially for filter dropdowns) - does not work for mssql...
            if($showUsedOnly) { 
            	$itemTables = array(212354=>"`client_contact`.`client_id`", 212366=>"`client_contact`.`client_id`", 212379=>"`project`.`client_id`", 212386=>"`project`.`client_id`");
            	if(isset($itemTables[$containerItemId])) {
            		$foreignParts = explode('.', $itemTables[$containerItemId]);
					$addSql = " INNER JOIN " . $foreignParts[0] . " dib___F ON dib___F." . $foreignParts[1] . " = `client`.`id` ";
            ...
            if($pageNoFromValue) {
            	...
            	$rst = dibMySqlPdo::execute("SELECT `client`.`name` AS dib__Display FROM `client` WHERE `client`.`id` = :pkey", DIB::$ITEMLISTDATA[3], array(':pkey'=>$pageNoFromValue), TRUE);
	            if($rst === FALSE)
	                return array('error', "Error! Could not read dropdown data information. Please contact the System Administrator");
				if($criteria) 
					$criteria = " WHERE $criteria AND `client`.`name` <= :dib__Display $order_by";
				else 
					$criteria = " WHERE `client`.`name` <= :dib__Display $order_by";
